...
| complete/FABLE_CI_FIXTURE.md | Handoff spec (sonnet): CI on GitHub Actions + synthetic fixture-DB generator — push-triggered tests/smoke/gui-check | closed 2026-07-21 (TODO-261) — B1–B6 all shipped same day as locked; ci.yml green on real GitHub Actions HEAD; tapematch-tests job dropped per tj (D-3, suite shells out to live sessions, unsafe/not CI-meaningful); found+fixed BUG-261/262/263 (pre-existing bloom-filter race + two background-thread leaks, the latter the actual CI segfault cause) along the way |
| complete/FABLE_ACTIVITY_CENTER.md | Handoff spec (sonnet): unified activity center (FABLE_IDEAS UI §4) — status-bar job tray over a normalized `/api/activity/jobs` aggregator + SSE tee registry | closed 2026-07-21 (TODO-262) — B1–B4 all shipped same day; backend/activity.py aggregator + SSE tee registry, busy re-based (D-3 gap workers now visible), gui_next status-bar tray + adaptive-poll store + i18n; 12 backend tests, gui-check PASS |
| complete/FABLE_COMMAND_PALETTE.md | Handoff spec (sonnet): Ctrl+K command palette (FABLE_IDEAS UI §1) — shared nav registry, fuzzy commands, LB/entry jumps, `registerCommands` extension contract for future specs | closed 2026-07-21 (TODO-263) — B1–B5 all shipped same day; `lib/navigation.ts` shared registry, `lib/commandRegistry.ts` extension contract, `lib/fuzzyMatch.ts` scorer, `components/CommandPalette.tsx`; gui-check green |
...
| complete/SPEC_INTEGRATION_NOTES.md | Cross-review / integration notes for the four Fable spec-pack docs | closed — whole pack shipped (RANKING 07-09, TAPER 07-15, ONBOARDING 07-10, LISTENING); §2 order executed as written, retained as historical record only |

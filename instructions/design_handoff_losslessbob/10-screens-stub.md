...
> **Update (build status):** Verify / Lookup / LBDIR are now **built as standalone advanced
> tools** in `feat/pipeline-v2-storage-mounts` (`ScreenVerify.tsx`, `ScreenLookup.tsx`,
> `ScreenLBDIR.tsx`) and they **stay** as power-tools. Their rich render bodies are the source
> for the Pipeline Workspace's stage panels — see `14-pipeline-gap-punchlist.md` §1 (harvest map):
> extract each into a shared `components/pipeline/*Detail.tsx` so the standalone screen **and** the
> pipeline panel render the same component. Do not maintain two divergent copies.

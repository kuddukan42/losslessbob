  ...
  return (
    <button
      ...
      style={{
        ...
        fontWeight: active ? 600 : 500, lineHeight: 1.5, whiteSpace: "nowrap",
        ...style,
      }}
    >
...
// ────────────────────────────────────────────────────────────────────
// FilterMenu — a dropdown button that opens a popover panel. Used to
// replace the old left filter rail: one button per facet group, the
// panel holds the group's selectable chips. Closes on click-outside / Esc.
// `count` shows an active badge; children may be a node or (close) => node.
// ────────────────────────────────────────────────────────────────────
function FilterMenu({ label, icon, count = 0, children, width = 256, align = "left", panelStyle, buttonStyle }) {
  ...
  return (
    <div ref={ref} style={{ position: "relative", flex: "0 0 auto" }}>
      <button type="button" onClick={() => setOpen(o => !o)} style={{
        display: "inline-flex", alignItems: "center", gap: 6,
        height: 30, padding: "0 9px 0 10px", borderRadius: 7, cursor: "pointer", fontFamily: "inherit",
        fontSize: 12, fontWeight: count > 0 ? 650 : 550, whiteSpace: "nowrap",
        background: count > 0 ? "var(--lbb-accent-soft)" : "var(--lbb-surface)",
        color: count > 0 ? "var(--lbb-accent-mid)" : "var(--lbb-fg)",
        ...
        <Icon name={open ? "chevUp" : "chevDown"} size={12} style={{ opacity: 0.55, marginLeft: -1 }} />
      </button>
      {open && (
...
// MenuLabel — small uppercase header inside a FilterMenu panel
function MenuLabel({ children, action }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
      <span style={{ flex: 1, fontSize: 10, fontWeight: 700, letterSpacing: 0.12, textTransform: "uppercase", color: "var(--lbb-fg3)" }}>{children}</span>
      {action}
    </div>
  );
}
...
function TR({ children, edge, selected, onClick, onContextMenu, style }) {
...
Object.assign(window, { Pill, Chip, Button, IconButton, Input, Kbd, Card, Toolbar, TableShell, TH, TR, TD, GroupRow, Stat, SectionHead, Banner, FilterMenu, MenuLabel });

...
> **CLOSED 2026-07-11** — all committed slots landed (both phases, incl. stretch).
> Only open remainder: TODO-184 (optional "fill (idle moments)" item — never
> scheduled; carries in TODO.md). Successor queue: `WORK_PACKAGE_NEXT.md`.
...
| **7/10: Olof P4** | ~~chronicles parser~~ **DONE 07-10 (0ec45d18)**: `backend/olof_chronicle_parser.py` + `olof_chronicle`/`olof_new_tapes` per FABLE_OLOF_FILES §6 P4 — 1,244 calendar rows (43 yrs), 79 new-tapes rows (17 yrs), 253 ok / 2 partial / 0 error, junk-free, idempotent, DSN untouched. **Finding: 2013+ chronicles are PDF-only on bobserve** → 2022+ appendix path dormant (0 synthetic events, structurally validated on 2002 A.htm); TODO-228 opened (PDF fetch+extract). NEXT Olof bite: P5 surfacing (endpoints, tour-name fallback TODO-290, setlist-vs-folder report, gui_next panel + i18n); spec §8 riders still deferred. | Medium | 162 partial (P1–P4); 228 opened |
| **7/10: Olof P5** | ~~surfacing~~ **DONE 07-10 (P5a bd48dd4e backend, P5b 3b9ca946 GUI)**: /api/olof/{date,event,chronicle,status} + POST /api/olof/compare (lb_number resolves entries.setlist, 11,796 rows); tour fallback setlistfm→olof +757 shows (**TODO-290 closed**); DetailPanel Olof tab both lenses (setlist/BobTalk/chronicle/provenance/compare, status-gated) + About credit (TODO-226 part B); i18n 5 locales, /gui-check PASS. **TODO-162 CLOSED — Olof spec P1–P5 complete, moved to instructions/complete/.** olof_* stays local-only (MASTER_TABLES export = open rights question for tj). Remaining Olof-adjacent: TODO-228 (2022+ PDF chronicles), TODO-226 part A (BobTalk search), TODO-224/225 riders. | Medium-high | 162 ✓, 153 ✓ |
| **Small bounded (idle)** | ~~TODO-210 implementation~~ **DONE 07-11** (2 sonnet agents, Fable-reviewed): (a) family-sync conf bump (+0.05 clamp, same scan_id + abs_score ≤0.5 + grade letter, feature-detected); (b) `duplicate_encode_candidates` + `--dup-encodes` CLI + `GET /api/tapematch/dup_encodes` (read-only; 15 pairs live incl. 3136/7538, 3147/7523; GUI → TODO-215). 26 new tests, suite 607 pass. **Bonus: TODO-227 closed same session** — crawl guard (30s backoff, 3×same-date → crawl_skip.txt honored by --next, 10× overall abort; Fable caught+fixed latent stale-$rc bug); live crawl untouched, guard active at next crawl restart. | Low | 210 ✓, 227 ✓ |

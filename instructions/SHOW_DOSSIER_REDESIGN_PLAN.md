...
>
> **Next: C30** (compact map: `marker` param on `_render_locator_svg`, ~240×150, coords from
> `view.venue.coords`). C29's template already emits `data-map-pin` / `data-venue-basis` on the
> venue card and renders the D1 `show.map_svg` there only when `venue.map` is usable — but the
> old SVG always draws a solid-looking pin, even where the attribute says `hollow`
> (1965-06-01). Lint L1 now runs on every html render and skips `data-claim` and
> `data-verbatim` (quoted source text) elements.
>
...
> (`.debug/c27_fragment_histogram.md`), **with a glued guard**: 3 of the 12 rank-1 picks it
> demoted were unsplit tracklists ("Serve Somebody 2. Million Miles 3. …") read as 0–1 songs,
> so an unparsed tracklist falls back to the runtime rule. The next sign-off is C32.
...
| C19 | 4 | D-03: draft `official_releases.json`, `official_release()`, rule R-R1 | C05, C06 | sonnet | D-03 accept cases. **Sign-off: allowlist**. **Signed off by tj 2026-09-11** — **all 104 rows reviewed** on the sign-off page: the 84 originals in one pass (83 correct, one wrong — the MTV Unplugged title, already fixed), then the 20 additions below in a second pass, every one approved — with additions: the allowlist stopped at 1989 for studio albums, so 20 entries were added (post-1989 catalogue, Street-Legal, The Basement Tapes, The Last Waltz, Renaldo And Clara, Concert For Bangladesh, 30th Anniversary, Complete Budokan 1978) → **104 entries (93 official / 11 not)**; 8 of 507 events with release text change status (6 none → full, 2 none → partial). MTV Unplugged retitled — it is its own 1995 release, not a Vol. 8 companion. The nine album patterns that are also song titles were **verified, not guarded**: `official_release()` only reads Olof's own release annotations, and 0 of the corpus's 19 "released as X on Y" constructions put an album-titled song in the "as" slot. Done: `official_release()`, R-R1 (535 open), `POST /api/qc/releases/<title_key>` + queue card. Multi-release strings take the release named earliest. `--d03` passes; **1975-12-08 reads `partial`, not `none`**: song 5 is on CD 14 of the 2019 Rolling Thunder Revue box | done |
...
| C21 | 4 | D-05 generation, D-13 medium (incl. the broadcast-taper R-T finding) | C08 | sonnet | D-05 / D-13 accept cases. **Sign-off: 100-row generation audit**. Done (sign-off pending): `classify_generation()`, `classify_medium()` (audio / audio_from_video + a broadcast flag, so an audience DVD keeps its taper), `media_available()`, rule R-T5 (4 open). `--d05` 6/6; the audit table is in `.debug/d05_audit.txt`. Rules decided by tj (2026-09-11): LB-08637's `bootleg_titles` row makes it silver (accept case changed from unknown); "master" followed by a clone is low_gen (451 LBs); a silver disc needs no label or catalogue number (+15). **The 100-row audit is signed off (tj, 2026-09-11)** with one widening and two explicit refusals to infer. Widened: `_MASTER_RE` matches the closed compounds (mastertape / masterdat / mastercopy — "more like a typo"), and `_CLONE_RE` matches the glued clone forms so the demotion still sees them (unknown 12,543 → 12,530, master/stated 1,002 → 1,014, low_gen 860 → 861). Refused: a master followed by a clone hop **stays** `low_gen` and nothing further is inferred about chain depth ("we definitely don't know master → clone1 → clone2 from any lineage info in our records"); ~~an unattributed mic → recorder chain stays `unknown`~~ — **reversed the same day** by tj's row-by-row verdicts on 72 of the 100 audit rows: 21 of his 26 corrections read "master inferred (microphone thru flac info given)", so the confirmed-taper gate is gone and a named recorder counts as the capture device on its own. Corpus: unknown 12,530 → 8,926, master/inferred 816 → 4,334. Guards added with it: an explicit chain gap ("DAT > ??? > Data DVD", 256 lineages) infers nothing, and a comma/slash shorthand lineage needs BOTH a mic and a recorder (54 LBs). All 43 rows he approved still classify as approved; 25 of 26 corrections satisfied (LB-16451's "ca14 m10" shorthand left alone). Excluded from the widening: mastered / mastering / masterpiece, and `masters` (8 of the corpus's 36 are the song "Masters Of War") | done |
...
| C23 | 4 | Supporting parsers (band, members, instruments, tally, writers, `set[].label`, character / flags, lineage_short, runtime, `family.basis`, taper render rule) | C04, C08 | sonnet | parser tests; runtime split sums to total. Done: 10 parsers in `backend/dossier_fields.py` (`parse_band_lineup`, `song_instruments`, `instrument_tally`, `song_writers`, `broadcast_set_labels`, `source_character`, `lineage_short`, `parse_runtime`, `family_basis`, `taper_render`), `--d23` **11/11 PASS**, 120 tests. G5 holds by construction — `entries.timing` never states a total beside its splits (0 of 12,341 rows), so `total_minutes` is the sum; 12,338 parse. Taper gate: the `error` rules R-T1/R-T2/R-T3 block rendering, the `warn` rules don't — R-T4 renders as the `disputed` notice (4,761 rendered / 11,942 nothing). Instruments and the tally read only numbered range / per-song clauses, never the base personnel clause (audit M13). `Solo` is Dylan alone in the base clause, not just the literal word — an unparenthesised band name ("with Tom Petty & The Heartbreakers") is excluded (band_index 3,006 / solo 243 / 1,371 lineups with named non-NET backing musicians, which render members only). `lineage_short` cuts at a DAW/codec token list compiled from the corpus, not an exhaustive one | done |
| C24 | 4 | D-08 pairwise comparison; D-10 bobtalk anchoring | C17, C22 | sonnet | D-08 / D-10 accept cases. Done: `compare_sources()` + `anchor_bobtalk()` in `backend/dossier_fields.py`, `--d24` **6/6 PASS**, 24 tests (144 in the file). D-08 diffs pick vs runner-up (`show_picks.pick_rank`) and scans every visible source for axis leaders; resolution never compares across a TUIT-disputed format. **Runtime floor decided (tj, 2026-09-12): 2 min**, the rounding floor — calibration in `.debug/dossier_calibration.md` (D-08) found runtime delta predicts no missing songs at any flat, per-era or relative floor, so the axis states "longest runtime" as a fact only; the 1965-06-01 accept case is amended from "collapses" to a +4 min runtime alternate (LB-03236, 70 vs 66). Corpus: 636 collapse / 3,198 compare; alternates scan 1,649, runtime 1,092, complete 375, soundboard 91, resolution 81. D-10 splits via `backend.bobtalk.parse_bobtalk`, resolves cues with `_PositionMatcher`: 668 events anchor ≥1 line (1,765 lines), 184 anchor none, 1,444 lines stay in Context; the raw cue regex hits 2,044 in 762 events today vs the plan's 1,862 / 677 | done |
| C25 | 5 | `dossier_anchors.py`: `Field`, full `ANCHORS` map (D-06 / D-12 as null stubs), `build_view`; `dossier["view"]`; `filter_dossier_sections(local_analysis=0)` | C17–C24 | sonnet | D1 payload unchanged (existing `test_dossier.py` passes); every anchor's fallback tested. Done: 96 anchors (61 scalar, 35 per-row under `song` / `source` / `family` / `set` / `run.dates`); view = `{fields, rows}`; `filter_view` strips `local_analysis` anchors. Claim-backed anchors resolve to fallbacks (C26); pick = existing rank-1, family label "Family A/B" (C27); `prov.withheld[]` empty (C28). Taper anchors go through `taper_render` (R-T1..3 block, R-T4 notice) with `reload_aliases=False` — reloading per source ran `init_db` 40–50× per build (20–70 s). Warm `build_view` ~750 ms on 1965-06-01 (18 sources); `family_basis` reloading abs scores per family is ~430 ms of it. `show.setlist_confidence` fallback is omit, not "complete". 270 tests in `tests/test_dossier_anchors.py` | done |
...
- **Accept:**
  ...
  - ~~1965-06-01 collapses.~~ **Amended (tj, 2026-09-12):** the runtime floor is the 2-minute
    rounding floor, so 1965-06-01's +4 min source renders as a "longest runtime" alternate.
    Runtime delta predicts no missing material at any floor (`.debug/dossier_calibration.md`,
    D-08); the axis states length as a fact and never implies completeness.

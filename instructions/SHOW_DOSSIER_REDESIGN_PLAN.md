...
> rules, `source[].group`), C26 (`backend/dossier_claims.py`, `view["claims"]`) and C25
> (`dossier_anchors.py`, `dossier["view"]`) landed 2026-09-12. C29's template must emit
> `data-lb`, `data-claim` spans and `data-map-pin` / `data-venue-basis` on the map — the
> `/api/dossier/html` route only runs `lint_l1` / `lint_data_lb` once `data-lb=` appears.
>

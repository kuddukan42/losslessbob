...
TODO-334: 1,414 catalogued recordings for run dates are missing from disk — build the acquisition worklist
Priority: Medium
Status: Open
Added: 2026-09-03
Description: Measured over the latest run per date on 2026-09-03: across the 3,062 dates tapematch has run, the LB database holds 13,468 entries but only 12,054 were found on disk, a shortfall of 1,414 recordings spread over 926 dates (30% of run dates). The reverse never happens — zero dates have more on disk than in the DB — so this is purely a collection gap, not a cataloguing one. Most gaps are small (594 dates missing one, 230 missing two, 71 missing three) but the tail is substantial: 1995-03-16 has 6 of 14 present, 1987-10-11 4 of 10, 1997-12-08 9 of 15, 2000-09-24 6 of 12, 1974-01-06 7 of 13, 1990-01-12 12 of 17. By decade the shortfall concentrates in the 90s (690 missing across 415 dates), then the 80s (321/224), 2000s (184/125), 70s (122/80) and 2010s (90/76). Two consequences, and the second is the one that has not been acknowledged anywhere: first, this is a ready-made acquisition worklist, already ranked by how much of a date is missing, and it should drive the torrent/TUIT sourcing effort instead of ad-hoc picking; second, every family verdict on those 926 dates is provisional by construction — the clustering only ever saw part of the date, so a missing recording cannot be matched to the tape it belongs to, and acquiring it necessarily invalidates the existing verdict. That interacts directly with TODO-333: a date that is stale on calibration AND incomplete on disk should not be re-run for calibration reasons until the missing sources arrive, or the re-run is wasted. Work: (1) emit the gap list as a persisted, refreshable artifact (date, missing count, which LB numbers) rather than a query someone remembers to run; (2) join it against the TUIT/torrent availability data now that 4,004 tuit_recordings rows map to LB entries (TODO-332) to separate 'obtainable today' from 'not in circulation', which is the split that makes the list actionable; (3) mark the affected dates' families as incomplete in the app so a user is not shown a family count that the pipeline itself knows is partial; (4) feed acquisitions back into the re-run queue shared with TODO-333/324/326.

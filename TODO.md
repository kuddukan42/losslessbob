...
Status: Open
Added: 2026-09-07
Description: Measured 2026-09-07 over tuit_recordings (4,809 rows, 4,792 claiming an LB number) against entries. The headline is agreement, not conflict: every LB number TUIT claims exists in entries (0 unknown), and on the 4,764 pairs where both sides carry a usable date, 4,761 agree. That makes TUIT a genuine independent check on entries rather than another noisy corpus, and the small residue is all actionable. (1) THREE DATE CONFLICTS, adjudicated against olof_events: LB-13944 (TUIT 1978-06-01 vs LB 6/3/78 — both dates are real Universal Amphitheater shows, so one of the two is misfiled), LB-13920 (TUIT 1993-09-19 Walnut Creek/Raleigh vs LB 10/9/93 Shoreline/Mountain View — different show entirely, so TUIT's lb_number claim is wrong, not the date) and LB-12610 (TUIT 2015-10-30 Southampton vs LB 4/17/17 Amsterdam — same shape). Two of the three look like bad TUIT attribution rather than bad LB data; confirm and, where TUIT is wrong, the tracker's lb_number is what needs correcting before any seeding decision trusts it. (2) 28 CIRCA-DATE REFINEMENTS: entries carries 'xx' dates (5/xx/94, xx/xx/61) where TUIT has an exact one — e.g. LB-00558 5/xx/94 → 1994-05-22 (The Great Music Experience), LB-06629 2/xx/61 → 1961-11-04 (Carnegie Chapter Hall), LB-00010 7/xx/87 → 1987-07-19 (Autzen Stadium). These are the cheapest date wins available and each is checkable against olof_events in one query. A few of the 28 are LB year typos rather than mere imprecision (LB-10879 5/xx/61 vs TUIT 2013-06-30; LB-02311 xx/xx/64 vs 1998-01-24; LB-04820 xx/xx/60 vs 1975-09-10), which is a separate and larger correction. (3) 69 SOURCE-TYPE CONFLICTS: 4,212 pairs agree AUD/Audience and 110 agree SBD/Soundboard, but 29 are TUIT SBD vs LB Audience, 22 TUIT AUD vs LB Soundboard and 18 TUIT FM vs LB Audience. A source-type disagreement on a specific recording is exactly the kind of thing the lineage text can settle, and it matters for the ranker. A further 284 pairs have a TUIT AUD label where entries.source_type is blank. (4) TAPERS: 1,529 agree, 629 LB-only, 601 differ and 582 are TUIT-only — i.e. TUIT names a taper on 582 recordings where entries has none, which feeds TODO-213's curation directly. Of the 601 differences only 184 (30%) are substring variants of one another ('robert71' vs 'robert71 16', 'Mike Millard' vs 'millard 1gen jems'); the rest are real disagreements including several where LB records a LABEL not a taper ('Crystal Cat', 'bootleg'), which is the known _KNOWN_TAPER_ALIASES problem. (5) 74 recordings on the tracker claim LB numbers whose lb_status is 'private' and one claims a 'missing' one — worth checking, since a private LB circulating publicly on TUIT is either a status error on our side or a leak. (6) VENUES: on the 2,524 circulating shows where entries has a location for the same date, the TUIT venue string appears inside entries.location 1,556 times; the other 968 are dates where entries.location is city-only ('omaha ne', 'rochester minnesota') and tuit_shows supplies the venue name we simply do not have. That is the concrete case for using tuit_venues as a normaliser — see TODO-338.

TODO-338: Cross TUIT's show spine against olof_events — 94 TUIT shows have no olof concert, 216 olof concerts have no TUIT row
Priority: Medium
Status: Open
Added: 2026-09-07
...
Status: Open
Added: 2026-09-03
Description: CONTRADICTED_CENSUS.md censused one polarity only — lb_says_same=1 with a different_family verdict (1,822 pairs, the recall problem). The opposite polarity has never been triaged: over latest_pairs on 2026-09-03 there are 114 pairs across 76 dates where lb_says_same=0 but the latest run merged them. These matter more per pair than the recall misses, because a false merge silently collapses two distinct tapes into one family in the app DB, whereas a miss only leaves them separate. The population splits cleanly by evidence type and should be worked as two separate queues, not one: 14 pairs sit at corr >= 0.75, where the primary correlation is decisive and the interesting question is whether the LB pages are wrong (a genuine undiscovered duplicate, worth a catalogue correction) — these are the high-value finds and there are few enough to listen to. The other 100 all sit below the 0.45 cluster threshold and therefore merged on a secondary, fingerprint or rule_d link, which puts them squarely in the failure class TODO-325 and TODO-319 are already chasing; expect most to resolve as false merges rather than as LB errors, and re-check them after whichever corroboration floor ships from TODO-325 rather than adjudicating them by hand first. Top dates by count: 1997-10-05 (6), 1999-03-02 (5), 2007-04-08 (4), 2009-04-11 (4), then 1990-06-01, 1995-05-26, 1997-10-03, 1997-12-11, 1999-07-10, 1999-10-26 at 3 each — note 2007-04-08 and 1990-06-01 are already named in TODO-319 and TODO-325 respectively, which supports the split above. Work: extend census_contradicted.py (or a sibling) to emit this polarity with the same bucket/marker treatment, run the 14 high-corr pairs past the LB commentary and a listen, and hold the 100 low-corr ones as a validation set for the TODO-325 floor — a floor that clears them without costing frozen-set true positives is a floor worth shipping. CENSUS BUILT 2026-09-03: tools/tapematch/census_false_merge.py, the mirror of census_contradicted.py. Artifacts: tools/tapematch/FALSE_MERGE_CENSUS.md (tracked) and data/tapematch/false_merge_queue.json (gitignored, per-queue pair lists with every signal, for re-running after a floor lands). Metadata only, no audio. Two corrections to the filing above, both from attributing the actual merge MECHANISM rather than reading a corr cut: (1) the '100 low-corr pairs' are not one queue. Replaying the shipped config.yaml through verdict.link_mechanism (new; pair_links now delegates to it, so the boolean and the named leg cannot drift — tests/test_link_mechanism.py) shows only 41 of them link on a direct leg — fingerprint_staircase 21, fingerprint 13, windowed 6, rule_d 1 — while 59 have NO direct leg fire at all and reached one family transitively through a third source. A TODO-325 floor can only act on the 41; the 59 are TODO-319's chain-clustering problem and no pairwise floor will touch them. (2) 30 of the 114 are not curator denials of this pair. lb_says_same=0 is set by extract_lb_relationship (tapematch_session.py:577) whenever backend.db._DIFF_RE matches anywhere in a ±250-char window around a mention of the other side's LB number, with no check that the denial is ABOUT the pair; in 30 rows the phrase is immediately followed by a THIRD LB number ('different recording than LB-3160' on a page that also mentions the pair's other side). Those rows are proximity artifacts and must not be counted against the matcher — the census carries a denial_scope field per row (84 pair_scoped / 30 third_party). The 14 primary-corr pairs confirm as filed (corr 0.82–0.99, all merged on primary) and none of the 14 is lineage-linked in entry_lineage, so LB genuinely does not record the relation; several are 1974 LB-2xxx/LB-9xxx pairs, which looks like a remaster/reissue family the catalogue never cross-referenced. REMAINING: listen to the 14 and decide catalogue corrections; re-run the census after a TODO-325 floor ships and check it clears the 41; hand the 59 to TODO-319.

TODO-335: Build a source-tape rarity census — 79% of Dylan's circulating tapes exist as exactly one copy
Priority: Medium
Status: Open
Added: 2026-09-03
Description: The clustering result has never been read as a preservation dataset, only as a matcher-accuracy dataset. Over the latest run per date on 2026-09-03 (3,062 dates, 12,034 sources) the pipeline resolves 9,131 distinct source tapes, an overall redundancy of 1.32 copies per tape — but the distribution is what matters, not the mean: 7,198 of the 9,131 families (79%) have exactly one member, then 1,349 at two, 356 at three, 137 at four, and a thin tail out to 10. Note that 7,198 is a LOWER bound on the true singleton count, because the known over-merge bugs (TODO-319, TODO-325) can only move tapes out of the singleton bucket and into larger families, never the reverse — so the finding is robust to those bugs even though the largest families are not (1999-10-26, 1997-12-18, 2002-04-28 and 2011-10-21 top the size table at 9-10 members and all four are named in TODO-319 as suspected over-merges, so treat the head of that distribution as unreliable until 319 lands). Redundancy also varies sharply by era, which is a real circulation signal rather than an artifact: sources per tape peaks at 1.86 in 1974 and 1.60-1.82 across 1995-97, sits near 1.15-1.30 through the late 80s and 2000s, and falls to 1.05-1.17 for 2017 onward, i.e. a modern show is captured once and shared once, where a 90s show circulated through several transfer generations. There are also 290 dates on which every recording present collapses to a single tape, meaning the date has redundancy in the collection but no independent second capture at all. Work: (1) materialise this as a per-date and per-family rarity metric in the app rather than a one-off query, since it is the natural input to the preservation spec in instructions/ (FABLE_PLATFORM_ROADMAP.md section 5, spec written and not started) — a singleton family with one lossy-format member on one disk is the highest-risk object in the archive and nothing currently names it; (2) cross the singleton set against dominant_ext and the integrity/mirror state to rank actual loss risk, not just copy count; (3) expose rarity on the show and family surfaces so a user can see that the tape they are looking at is the only one that exists. Do not present the redundancy figure to users until TODO-319 lands — over-merge deflates the family count and therefore inflates the redundancy ratio.

TODO-334: 2,069 recordings are on disk but structurally excluded from tapematch — the shortfall is an analysis gap, not an acquisition gap
Priority: Medium
Status: In progress — gap list built 2026-09-03, policy decision open
Added: 2026-09-03
Description: PREMISE CORRECTED 2026-09-03. This was filed reading the 1,414-recording shortfall between n_sources_db and n_sources_found as an acquisition worklist. It is not one. tools/tapematch/build_gap_list.py now resolves the shortfall per LB number instead of per count, and over the 3,061 run dates (14,215 catalogued recordings) it splits as: present 12,017 (resolved and ingested), private 2,069, unranked 110, absent 19. Only those 19 are actually not on disk, across 18 dates, and 18 of the 19 have no my_collection row at all (LB-01881, LB-05611, LB-04162, LB-01808, LB-01795, LB-01822, LB-05119, LB-05120, LB-05117, LB-05126 and the rest are in the generated report) — so the real acquisition list is small enough to work by hand in one sitting. The 2,069 'private' recordings are the actual finding: they are on disk WITH audio, and find_lb_folders (tapematch_session.py:431) drops them before the run because their path matches PRIVATE / NOTORRENT / NO TORRENT. The stated reason is that a private/no-torrent entry has no local LB page and therefore no curator commentary to corroborate a merge against. That is a defensible rule for a workflow whose verdicts lean on commentary, but nobody has costed it: it means ~15% of the catalogued corpus on run dates has never been matched at all, concentrated in the 1990s (1,157 of the 2,069) and the 1980s (316), and 87 dates have at least as many private recordings as ingested ones. Worst-affected dates are 1995-03-16 (6 of 14 ingested), 2000-09-24 (6 of 12), 1997-12-08 (9 of 15), 2000-03-17 (7 of 13), 1974-01-06 (7 of 13) and 1974-01-31 (26 of 31); 1,364 of the 3,061 run dates have at least one unseen recording. DONE: build_gap_list.py written, with a mount guard that refuses to write when a DYLAN drive is unmounted (every 'absent' verdict is a filesystem existence test, so an unmounted drive would invent thousands of false acquisition targets). Artifacts: tools/tapematch/GAP_LIST.md (tracked summary) and data/tapematch/gap_list.json (gitignored, machine-readable, per-date bucket lists). Metadata only, no audio decoded, cheap to re-run. REMAINING WORK: (1) the policy decision, which is the whole of this TODO now — should tapematch ingest private/no-torrent sources and simply mark their families as commentary-uncorroborated, rather than not measuring them at all? Correlation does not need an LB page; only the corroboration step does. Weigh that against the over-merge work (TODO-319/325), since admitting 2,069 uncorroborated sources into clustering without commentary to check them against would land in exactly the failure mode those TODOs are chasing — likely gate it behind the TODO-325 floor rather than shipping it first. (2) Source the 19 absent recordings, and cross them against tuit_recordings (TODO-332) to see which are obtainable. (3) Split the 110 'unranked' — the census cannot tell 'folder has no analyzable audio' from 'catalogued after the run' without walking the folder; a re-run of those dates resolves both and they should go into the shared re-run queue with TODO-333/324/326. (4) Mark affected dates in the app so a family count on a high-unseen date is not presented as the number of source tapes that exist.

TODO-333: tapematch — the shipped family verdicts are a patchwork of calibration eras, with no way to tell which are current
Priority: Medium
Status: Open
Added: 2026-09-03
Description: Measured over observations.db on 2026-09-03, after the analysis backlog drained (next_batch.py returns empty; 3,107 analysis.md files across 4,102 run dirs, the remainder being incomplete sets). Taking the latest run per date — 3,062 dates, 12,034 sources, 9,131 families — the config_json of those latest runs hashes to more than ten distinct configurations, the largest covering only 876 dates (then 811, 668, 296, 202, 92, 81, and a tail of single digits). 585 dates were last run in 2026-06, i.e. before staircase 0.40, before the TODO-255 frozen-set gating and before rule_d fired live at all; 2,384 in 2026-07 and 93 in 2026-08. So recording_families in the app DB does not represent one algorithm's opinion, it represents whatever the pipeline believed on the day each date happened to be run, and nothing in the schema records which. Corroborating signal: of the 8,457 pairs observed more than once, 1,572 (18.6%) have had their verdict flip between runs, and 1,464 of those flipped on the most recent run — consistent with calibration improvements landing unevenly rather than with noise. The other ~16k pairs have exactly one observation ever. Work: (1) record calibration identity per run — a stable hash of the verdict-relevant config keys, not the whole config_json blob, since archive paths and unrelated keys currently split otherwise-identical runs; store it on runs and carry it through tapematch_sync.py to tapematch_family_meta so a family can be asked which calibration produced it; (2) define what 'current' means — most likely the shipped config.yaml's hash — and build a staleness report that counts dates per calibration era; (3) prioritise the re-run queue by expected verdict change rather than by age, using the flip history above: the dates whose verdicts moved when a config changed are the ones most likely to move again, and the June-2026 cohort predates the three largest precision changes; (4) surface staleness in the GUI so a family shown to a user carries its provenance instead of implying it is fresh. Do NOT bulk re-run all 3,062 dates — the point of the exercise is to identify the subset whose verdict actually turns on the calibration difference. Overlaps TODO-324 (a different staleness axis: verdicts resting on the pre-BUG-330 untrusted speed ratio) and TODO-326 (superseded runs); a single re-run queue should serve all three rather than three separate sweeps. STEPS 1-3 SHIPPED 2026-09-03. (1) tools/tapematch/tapematch/calibration.py defines the calibration identity: DECISION_KEYS (every config key that can change a verdict, each with its code default), GATED_BLOCKS (a disabled block collapses to its off switch, so rule_a at t_flaw 0.45 vs 0.60 is one calibration while the rule is off), NON_DECISION_KEYS (device/threads/paths), and VERDICT_ONLY_KEYS (the subset verdict.py reads, i.e. thresholds over already-computed metrics). calibration_hash() is 12 hex chars over the normalised view. Stored on runs.calibration_hash (tapematch_session.py, PRAGMA migration) and carried through tapematch_sync.py to tapematch_family_meta.calibration_hash, so a family can now be asked which calibration decided it. A drift test requires every shipped config key to be classified deliberately. (2) tools/tapematch/calibration_eras.py --backfill/--rehash fills the column and writes tools/tapematch/CALIBRATION_ERAS.md. Result: the normalisation collapses 18 raw config_json hashes to 11 real eras, and the shipped config (0e764850fc22) covers 768 of 3,062 dates (25.1%). The two largest stale eras are narrow, not sweeping: 76f78b8480f3 (876 dates) differs in ONE key (staircase_corroboration.min_hiss_median absent vs 0.05) and 31b4465ce221 (811 dates) in four (the staircase_corroboration block off vs on). (3) The re-run queue is now built by expected verdict change rather than by age, and the result kills most of the apparent problem: 1,687 stale dates differ from current ONLY in VERDICT_ONLY_KEYS, so their stored pair metrics can be re-decided exactly by replaying verdict.cluster_verdicts under both configs — no audio. Of those 1,687, just 33 change verdict; the other 1,654 are bookkeeping-stale and a re-run buys them nothing but a fresher hash. The 33 are listed in CALIBRATION_ERAS.md with their pair-move counts (1997-10-03 at 15 pairs, 2009-04-11 at 6, 2010-11-22 and 2014-06-27 at 5). The remaining 607 stale dates sit on eras that also differ on signal-generation keys, so they cannot be replayed and need real re-runs — that is the queue to share with TODO-324/326. REMAINING: (4) surface calibration_hash in the GUI so a family carries its provenance (needs GET /api/tapematch/families to return the column); re-run the 33 movers and the 607 unreplayable dates; and re-run calibration_eras.py --rehash whenever DECISION_KEYS changes, since that changes what every stored hash means. Note the confound to respect when reading any staleness count: 926 of the 3,062 dates ran on an incomplete set (13,468 LB entries for those dates vs 12,054 found on disk, 1,414 missing), so their family counts are provisional regardless of calibration, and re-running them under a current config will not make them correct.


TODO-332: Reconcile TUIT recording metadata against our entries — venue, lineage, setlist, source type, dates
Priority: Medium
Status: Open
Added: 2026-09-03
Description: TUIT (tangledupintorrents.org) metadata is structured where ours is free text, and 4,004 of 4,017 scraped tuit_recordings rows map to an LB entry (13 unmapped; covers 24% of 16,703 entries). A 2026-09-03 comparison of the two sets found: DATES — 3,974/3,977 full dates agree (99.9%); 3 real conflicts to adjudicate (LB-12610 4/17/17 vs 2015-10-30, LB-13920 10/9/93 vs 1993-09-19, LB-13944 6/3/78 vs 1978-06-01); TUIT also resolves 18 of our 27 'xx' partial dates. VENUE — TUIT has a structured venue for 100% of rows while our entries.location is free text that contains the venue name in only 1,238/4,004; this is the largest single gain and pairs with the existing olof_events.venue rule (use venue, not location strings, for show identity). LINEAGE — TUIT 3,268 vs our source_chain 2,258; 1,023 rows have lineage only on TUIT. SETLIST — TUIT 3,786 vs ours 3,302; 652 TUIT-only. SOURCE TYPE — 3,628/3,724 agree (97%), 280 rows TUIT fills where we have none, 96 conflicts that are mostly AUD<->SBD flips plus TUIT classes our vocabulary lacks (Matrix, Radio). TAPER — already covered by the taper curation workflow (backend/taper_curation.py, TODO-213/312); note only that our taper_name is polluted with non-taper words ('bootleg', 'master', 'digital master') where TUIT holds a real handle ('Crystal Cat', 'RL'), which depresses the apparent 1,537/1,861 (83%) agreement and is worth a cleanup pass. Work: decide per field whether TUIT is authoritative, additive-only, or advisory; add provenance so an imported value is attributable and reversible (entries.metadata_source already exists); build a reconciliation surface that lists conflicts for human adjudication rather than overwriting silently; and reuse the taper curation console pattern instead of inventing a second review UI. Do NOT bulk-overwrite entries from TUIT without that review step.
...
2026-09-02 sweep (full tables in tools/tapematch/CALIBRATION_PROGRESS.md '2026-09-02 TODO-325'): answered as 'a corroboration requirement, scoped by MECHANISM'. Two dark config keys now exist in verdict.py, both absent from the shipped config: match.secondary_primary_floor (uniform - windowed/hiss/fp/triplet) and match.fingerprint_primary_floor (fp base/staircase/curator + triplet only). The UNIFORM floor is unusable: 1980-12-04 is a genuine taper-corroborated merge on a windowed leg at corr 0.0215, LOWER than BUG-331's false fp merge at 0.0295, so no floor separates them. The fp-scoped floor does, because the two link through different legs. At 0.10: fixes BUG-331 (2008-07-08) and all three 1990-06-01 pairs, zero new frozen-set FPs, tp 705->614 / fp 18->7, preserves 1980-12-04, 1993-10-03 and 1995-09-27's core pair (only LB-07477 drops out of that family). 0.05 makes the same two fixes for -50 tp / -7 fp. Out of reach either way: 2024-03-05, 2024-11-08 and 1981-06-21 link via addon_links.rule_d (embedding, deliberately ungated - gating it costs 150-230 tp), and 2023-04-20 links on a windowed leg at corr 0.390. 1995-06-16 turned out to be already fixed by staircase_corroboration. REMAINING WORK: tj picks 0.10 or 0.05 (or neither) and the key gets uncommented in config.yaml.

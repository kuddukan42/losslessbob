...
Progress (2026-08-22): step 1 COMPLETE — all 43 affected dates re-run (tools/tapematch/bug326_rerun_queue.txt, every line marked done; data/tapematch/bug326_rerun.log ends 'Batch complete: 43 date(s) processed'), producing 43 new run dirs data/tapematch/runs/20260821_*. The scan that built the queue is now kept as tools/tapematch/bug326_scan.py (was a tools/_ throwaway) since this entry calls for regenerating the list rather than trusting a stale copy. REMAINING — step 2, the point of the task: diff the new family assignments against the pre-fix run for each date and re-write only the analysis.md files whose verdict actually changed. Not started. Note 26 of the 43 new dirs already carry an analysis.md (written fresh by the nightly /tapematch-batch cron, which does not diff against the superseded run), so their presence is NOT evidence step 2 was done; 17 have none.
Step 1 (re-run) completed 2026-08-21: all 43 affected dates reprocessed with the fixed ingest. Step 2 (verdict diff) completed 2026-08-22 with the new tools/tapematch/bug326_diff.py, which parses the CLUSTERS block of each post-fix run and its newest pre-fix predecessor and compares the LB partitions. Of the 43 dates: 33 SAME (family assignment identical after de-dup, so their existing analysis.md verdicts stand untouched — this is the finding that kept the rewrite scope small), 5 CHANGED, 4 NO-PRIOR (1965-06-01, 2001-08-15, 2014-10-18, 2014-10-28 had no earlier run to invalidate), 1 UNUSABLE (1991-06-21's pre-fix run was incomplete and has no CLUSTERS section, so there was no prior verdict to compare; its post-fix run IS complete and now carries no analysis.md, so /tapematch-batch will pick it up normally). Of the 5 CHANGED, 4 had a stale analysis.md and were rewritten: 1969-08-31 (LB-05444 split out of LB-05457+LB-07385, resolving that run's own open question about which LB owned the Family 2 correlation), 1989-06-21 (LB-02142 merged into LB-06976+LB-08351, 3 families -> 2, confirming the taper commentary the pre-fix run contradicted), 1990-11-10 (LB-08282 pulled into LB-01201+LB-01215, 6 -> 5, confirming one of the three 'same clapping wavs' claims the pre-fix run had dismissed), 2012-07-03 (Family 3 broke up, 5 -> 7, confirming the over-merge that run's analysis had already suspected across four separately-credited tapers). Each rewritten file keeps its original table as the record of what the pre-fix run said, restates the verdict as needs-review/superseded, and points at the post-fix run's analysis.md as authoritative. The 5th CHANGED date, 1981-06-29, needed no rewrite: it had no analysis.md on either side, and its change is NOT a de-dup effect (no source shrank; a new source LB-16657 entered the corpus between the two runs). recording_families/tapematch_family_meta already reflect the post-fix clustering (backend.tapematch_sync re-run 2026-08-22: 3,062 dates, 9,132 families, 12,035 recordings, 0 errors).
...
Added: 2026-08-17
Closed: 2026-08-17
Description: Turn the conflict-only /taper-review queue into the full curation console. The page could
only reach the ~96-row mention-conflict slice; the other ~8,550 attributions (273 handles) had no
review surface, no grouped view, no search and no decision history. Adds: taper_decision_log
(USER-tier audit trail with prev_action/prev_taper, making decisions revertible without a
recompute); list_review_rows/taper_rollup/list_decisions/revert_decision in
backend/taper_attribution.py; five routes (GET /api/tapers/review, /api/tapers/review/tapers,
/api/tapers/decisions; POST /api/tapers/attributions/bulk, /api/tapers/decisions/<id>/revert);
and a three-tab rewrite of backend/taper_review.html (Queue / Entries / Tapers) with facet chips,
search, pagination, multi-select bulk confirm/reject/unresolved, per-row evidence + history +
revert, hash-linkable views, and a wholesale curator gate.
Shipped 2026-08-17. Backend: taper_decision_log
table (USER_TABLES, idempotent DDL + idx_taper_declog_lb); _log_decision() wired into confirm/reject/
mark_unresolved inside their existing transactions so CLI callers are logged too; revert_decision()
replays prev_action/prev_taper (and, for a first-ever decision, drops both the confirmation and the
derived row, returning needs_recompute:true); list_review_rows/_review_counts/taper_rollup/
list_decisions. entries.date_str is M/D/YY, which sorts and MIN/MAXes wrong as text, so date sort and
the rollup span go through a SQL rewrite to YYYY-MM-DD (two-digit year pivots at 40). Five routes
added, reads open and writes curator-gated, matching the existing taper routes. Page rewritten as a
three-tab console. Verified: 40 tests in tests/test_taper_attribution.py (full suite 1313 green);
migration idempotent against the 1.25 GB production DB with integrity ok and row counts unchanged;
route totals match direct SQL (8,566 undecided, 8,062 unattributed, 273 handles, 96-row mention queue
identical to the old endpoint); end-to-end bulk confirm -> history -> revert round-trip returned
taper_confirmations to its 109-row baseline; headless-Chromium pass over all three tabs, the taper
drill-down and the bulk bar with zero JS errors. Note: a full taper_attribution.recompute() was run
during verification, moving the derived table 8,641 -> 8,646 rows and 131 -> 136 conflicts (it is
USER-tier and rebuilt wholesale by design).
...
Closed: 2026-08-17
Description: Census + item(c) both complete. Full 1,822-pair corpus bucketed (44.2% alignment_failure -> TODO-204, 34.2% label-noise markers -> TODO-201, patchwork only 2.9% confirming TODO-185's cancellation was correct). Item (c)'s 377 unexplained pairs resolved via emb second pass: 90% sit in the negative-control band, not a hidden failure class. Surfaced BUG-278 (rule_d dead live) as a byproduct, now also closed. Remaining work belongs to TODO-201 and TODO-204, not this task.
...
Shipped backend/checksum_provenance.py: classifies every LBF-* attachment in data/site/files/ as a checksum source (lbdir vs uploader, self vs xref, suspect-name detection), re-parses it with parse_lbdir_file, and compares against the checksums table by (lb, basename, chk_type). Isolated disagreements (source agrees on >=3 rows, disagrees on <=25%) are recorded as kind=db_mismatch — the corrupted-DB-value signature; whole-set disagreements are recorded as kind=set_divergence (a different version filed under the same LB) and never presented as DB errors. New MASTER table checksum_disputes holds the findings plus a curator verdict (open/confirmed/dismissed) that survives re-running the audit. lookup_checksums() now indexes source_checksum, so a NOT FOUND explained by a known-bad DB value comes back annotated with dispute{lb_number, db_checksum, source_file, confidence}. CLI: lb checksum-audit [--lb N] [--list] [--all] [--include-lbdir]; routes GET /api/checksum-disputes and PUT /api/checksum-disputes/<id>. Catalogue: 98,718 attachments over 15,205 LBs; 34,502 non-lbdir files carry 744k hash rows over 14,357 LBs; provenance was not tracked anywhere in the schema before this. First full pass (8s over 62,184 files): 287 db_mismatch rows (188 high / 99 medium) and 9,627 set_divergence rows; 78 of the high-confidence source values are absent from checksums entirely, so those users currently get a bare NOT FOUND on a good file. LB-15933 itself produced no dispute — its lbdir manifest, its .ffp attachment and the DB all agree on d1t14, so that report needs a source outside the mirror. Triage of the 188 findings and GUI surfacing left to TODO-299.
...
Xref incorporation complete per instructions/FABLE_XREF_INCORPORATION.md: B1 (lookup matched_xref/xref_groups), B2 (naming/schema/pipeline), B3 (gui copy-level pill + Cross-refs bar), B4 (gui entry-level surfaces, filter split), B5 (dff_reports keying), B6 (cli cleanup), B7 (PROJECT.md docs) all shipped. Definition of done verified: fixture LB-2 xref-961 returns MATCHED + matched_xref 961 end to end; /gui-check green; 826 backend tests pass; entry-level vs copy-level use distinct i18n keys. B8 site-mirror ingest NOT built per D-2 default (master import remains the only checksums write path) — revivable if tj promotes it.
...
Added: 2026-07-15
Closed: 2026-07-15
Description: Drive the real gui_next Electron app (not just Chromium vs the Vite build) so screens, window resize, display scale and progress meters can be verified as PNGs on this machine. 4 bites per spec §8; THREE AND A HALF ARE DONE — only bite 3b is left, and this entry stays open solely for it. Bite 1 DONE 2026-07-15 (tools/electron_preflight.mjs, probe matrix, Xvfb selected + recorded in tools/electron_driver.config.json). Bite 2 DONE 2026-07-15 (tools/electron_driver.mjs MVP + tools/driver_core.mjs shared action runner + LB_NO_BACKEND_SPAWN gate). Bite 3a DONE 2026-07-15 (resize/size-matrix/scale-matrix/watch/main-eval via a shared `caps` object; matrix PNGs verified exact; findings 9-11). Bite 4 DONE 2026-07-15 (/verify --electron mode in .claude/skills/verify/SKILL.md, all 7 driver files added to PROJECT.md tools/ listing, CLAUDE.md exception note, spec moved to instructions/complete/; full 17-screen Tier B tour re-verified). Acceptance criteria 1/2/3/5/6 MET. REMAINING — bite 3b, the dev-gated progress fixture, which is the whole of unmet criterion 4 (watch a meter fill 0->100%): read spec finding 12 FIRST — the meter is FileProgressBar (/api/pipeline/file/start + /file/status, _FILE_JOB in backend/filer.py), NOT the /run/status per-folder job; feeding _FILE_JOB alone fails twice (the screen renders the bar off client state set by clicking File, and a pre-armed job makes the real start_file_job return `busy`), so it needs an intercept INSIDE start_file_job past its busy/stale-verify/resolve_destination_for_lb guards, a simulated _run() with no filesystem IO, and a staged row with a real folder + valid LB in entries (status.path must match the row's folder path or ScreenPipeline.tsx:1968 reports a job mismatch). This is a backend design job, not driver plumbing — design it with tj before writing code; do not let an agent improvise it. Verification stays user-invoked only. Full spec: instructions/complete/FABLE_VISUAL_VERIFICATION.md.
DONE 2026-07-15 for bites 1, 2, 3a and 4 — /verify --electron drives the real Electron app on Xvfb (screen tour, size-matrix, scale-matrix, watch, main-eval); acceptance criteria 1/2/3/5/6 met. Bite 3b (dev-gated progress fixture) CLOSED WON'T-DO by tj sign-off 2026-07-15: not enough animation in the app to be worth it. Consequence accepted: acceptance criterion 4 (watch a meter fill 0->100% against a fixture) is permanently unmet, and the FileProgressBar mid-fill state is not machine-verifiable. What that cost would have been: a test-only intercept inside start_file_job in backend/filer.py past its busy/stale-verify/resolve_destination_for_lb guards, a simulated _run(), and a staged pipeline row — production code touched to animate one meter (spec finding 12). The 'watch' driver action itself is BUILT AND WORKING (bite 3a) and still runs against real jobs; only the synthetic fixture is dropped. Revisit only if a future screen has progress/animation worth machine-verifying. Spec: instructions/complete/FABLE_VISUAL_VERIFICATION.md.
...
CLOSED 2026-07-15 (tj sign-off: remaining items not worth the value). Core pipeline shipped and in production since 2026-06/07: scan/calibrate/rerank/report CLI, per-decade + per-class bands, AUD model (CV Spearman 0.66) + SBD model (0.56), DFF integration, filtering + hard HF floors, 13,752 recordings scored, GUI Quality tab. Closing action: sibilance_ratio_db demoted to informational (POLARITY 0, SEVERITY_BANDS entry + tonal-family fusion removed) — never de-confounded from brightness (+0.34 rho above 9 kHz ceiling; hf_ceiling artifact below); sibilance_crest (validated, rho -0.34..-0.65) kept as the sibilance defect signal, polarity -1. WON'T-DO: sibilance_ratio_db 9kHz-gate rescan, SBD-per-decade bands (sparse), pop/click detector, DFF-on-Linux, lossy_flag calibration, dynamic_range_dr production, band-label phrasing polish. 49 tests pass.
...
Won't-ship (tj decision 2026-07-15): Layer 2 fully implemented, tested (tests/test_taper_fingerprints.py, 44/44) and calibrated, but gated OFF via taper_fingerprints.LAYER2_ENABLED=False — 96.2% holdout precision does not transfer to the unattributed pool (~60-75% by spot-check; profiles latch onto era/setlist vocabulary, formatting style, format boilerplate; era-matched and gear-only variants prototyped, leakage persists). Revisit options documented in WORK_PACKAGE_2026-07-14.md Session 6 (rare-token evidence gate, or curator suggestion queue); TODO-213 curation growth may improve the source set. Spec FABLE_TAPER_ATTRIBUTION.md moved to instructions/complete/.
...
PROGRESS 2026-07-14 (conflict queue cleared): split the queue by kind — added `_is_series_vs_series` + a `conflict_kind` filter (`list_attributions`) and pointed /taper-review at `conflict=1&kind=mention` so the 22 series-vs-series (TODO-234, un-pickable in the hand queue) no longer appear. Added a "can't determine" verdict for genuine historical two-taper conflicts (same recording, two documented tapers, no ground truth): `mark_unresolved()` + `POST /api/tapers/attributions/<lb>/unresolved` + `_apply_unresolved` (sticky across recompute, suppresses every taper → no pill), and a review-page button. Bulk-parked all 31 mention-vs-mention as 'unresolved' (DB backed up: data/backups/…_pre_unresolved_bulk.db) — hand queue now EMPTY. taper_confirmations ledger: 58 confirm / 10 reject / 31 unresolved. Attribution-curation portion of TODO-213 is DONE; what REMAINS under TODO-213 is the show-pick/badge WEIGHT-TUNING pass (needs tj's concrete wrong-badge examples first) and pushing the finalized curation into badges via /api/derived/recompute. The 22 series-vs-series stay open under [TODO-234].
Done 2026-07-14. The attribution-curation half shipped (non-taper credit filtering, mention-downgrade, cb-master rename, conflict-queue split by kind, 'can't determine' verdict; mention queue cleared to empty). The remaining show-pick/badge WEIGHT-TUNING pass is deliberately NOT pursued: tj's call — only ~22 taper-badge conflicts remain, all series-vs-series and well understood, owned by [TODO-234] (tapematch family over-merge), not a weight-tuning problem. Taper badge state is trustworthy enough; no further action needed here.

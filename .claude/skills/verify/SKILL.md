...
| | Tier A (default) | Tier B (`--electron`) |
|---|---|---|
| Driver | `tools/browser_driver.mjs` | `tools/electron_driver.mjs` |
| Runs | Playwright Chromium vs the Vite build | the REAL app (`gui_next/out/main/index.js`) on Xvfb |
| PNGs | `.debug/` | `.debug/electron/` |
| Extra actions | — | `resize`, `size-matrix`, `scale-matrix`, `watch`, `main-eval` |
...
`--electron` when the question is about the real window rather than the page:
window chrome, resize/layout at real sizes, display scale, native
`window.api`/preload flows, or main-process state. Both tiers share the same
session format (`tools/debug_screens.json`), so the screen tour is identical.

Design/history for Tier B: `instructions/complete/FABLE_VISUAL_VERIFICATION.md`.
...
1. Ensure the Flask backend is running on port 5174 (the GUI needs it for live
   ...
   ```
   Tier B always sets `LB_NO_BACKEND_SPAWN=1`, so the app will not spawn its
   own backend — a manually-started one on 5174 survives the whole session, and
   the driver logs backend status before and after the run.
2. Run the full screen tour (builds gui_next first):
   ```bash
   # Tier A (default) — screenshots land in .debug/
   node tools/browser_driver.mjs session tools/debug_screens.json

   # Tier B (--electron) — screenshots land in .debug/electron/
...
- Single screen: `node tools/browser_driver.mjs navigate <route> <file.png>`
  (or `electron_driver.mjs` for Tier B).
- Tier A stubs `window.api`, so file-picker flows can't be exercised there —
  note them as "not verifiable" rather than FAIL, or re-run that screen under
  `--electron`, where the bridge is real.
- Tier B extras, all `.debug/electron/`:
  ```bash
...
- Tier B never re-probes display backends — it reads the Xvfb decision recorded
  in `tools/electron_driver.config.json`. If it errors that no backend is
  `selected`, run `node tools/electron_preflight.mjs` and own the decision;

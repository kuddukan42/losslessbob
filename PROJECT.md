...
| GET | `/qc-review` | Serve the QC review console (`backend/qc_review.html`, TODO-342 Phase 2b, experimental, English-only) — self-contained like `/taper-curation`. Header: open errors · open warnings · last run · **Run rules** (posts `/api/qc/run`, polls to completion). **Queue** (default tab): one open finding at a time (error before warn, then rule_id, then id; optional rule filter), rule-family evidence panel (taper mention highlight + Reject attribution + Open in /taper-review for R-T; Olof raw vs parsed for R-O; geocoder match for R-G1; raw row dumps for R-E1/R-F1/R-S1), affected-shows Open-dossier links, decision row (Confirm/False positive/Correct…/Skip + note), keys `c f e s j k o`. Current id + skipped ids persist to `localStorage` (try/catch-wrapped) so a reload resumes. **Findings**: `qc_findings` workbench with rule/severity/status chips, search, pager, detail drawer, and multi-select bulk Confirm/False-positive (one rule per batch, else disabled). A curator pill reflects `/api/curator`; any 403 surfaces "curator mode is off", not a silent failure. |

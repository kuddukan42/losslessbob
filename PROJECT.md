...
│   ├── qc_review.html        # QC review console (served at /qc-review, experimental) — Findings workbench over qc_findings; Queue tab + writes in C12 (TODO-342)
...
| GET | `/qc-review` | Serve the QC review console (`backend/qc_review.html`, TODO-342 Phase 2b, experimental, English-only) — self-contained like `/taper-curation`. Header: open errors · open warnings · last run · Run rules (disabled until C12). **Findings**: `qc_findings` table with rule/severity/status chips, search, pager and a detail drawer (evidence, context panel, Open dossier links). **Queue**: placeholder until C12 adds the card flow and curator-gated writes. |

...
| POST | `/api/refresh/chain/preview` | Body `{step_id?, trigger?, include_expensive?}` (exactly one of `step_id`/`trigger`). `backend.refresh_exec.plan_chain()` — read-only DAG walk from the seed pulling in stale ancestors; claims nothing, starts nothing, safe to call on every keystroke. 400 on a bad seed. TODO-310 P4: also returns `advisories[]` (`{queue_id, count, step_id, kind queue/publish}`) for the preview dialog — advisory text only, never a gate. |
...
| Screen | File | Status |
|--------|------|--------|
| ScreenHome | `screens/ScreenHome.tsx` | Done — dashboard, live stats, activity log, flat-file update; first-run onboarding (TODO-217): auto-opens `components/OnboardingWizard.tsx` (4-step modal: master install → sitedata install → mounts/pipeline nav → done + `/api/derived/recompute`) once per launch when `entries_count == 0`, plus a setup-checklist card while `/api/onboarding/status` `complete == false`; `components/DataFreshnessCard.tsx` renders the `/api/refresh/status` rollup (stale/blocked steps grouped by trigger, publish-lag callout, muted no-signal footnote) and hides itself entirely when the route is absent or errors; TODO-306 P2: `olof_fetch`/`bobserve_fetch`/`ranker_scan`/`ranker_rerank` get a real "Run" button (`RUNNABLE` map) with a `ConfirmDialog`, 2s status polling + Stop button while running, and a `version.state=='changed'` tooltip hint — every other row keeps the Phase 1 copyable-text/nav-button behaviour. TODO-310 P4 adds a **queue panel** below the trigger groups: one row per `gate` queue (warn pill when pending, muted `✓` when clear — never hidden, so an empty queue can be *confirmed* empty — plus its action line and a "Review →" deep link), one ratio + thin progress bar for the `backlog` queue (no pill, no colour), and a `⚑ N to review` marker after the state chip on any step a queue names. `xref_filesets` is display-only (count and explanation, no Review button, and the card calls no `/api/xref_ingest/` route). Chain-preview advisories render above the confirm button, which stays enabled. `AppShell.tsx` polls `/api/refresh/queues` every 60s and badges the Library and Fingerprint nav items via `lib/navigation.ts`'s `NAV_QUEUE_BADGES` (TapeMatch and xref are deliberately unbadged) |

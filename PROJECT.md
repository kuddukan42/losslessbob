...
| GET | `/api/tapematch/dates` | Left-rail summary, date DESC: `{dates: [{date, run_id, n_lbs, n_pairs, has_analysis, needs_review, location, curated, curated_at}]}`. `location` resolved via the date's LB numbers (entries.date_str is US-format, never joined on). has/needs fields null when observations.db missing/locked. `curated`/`curated_at` come from observations.db's `curation_accepts` (see the accept route below) and degrade to `false`/`null` when the table doesn't exist yet or the DB can't be read — the curation screen's fourth triage state, which wins over conflict/review because it is the curator's own terminal verdict. |
| POST | `/api/tapematch/dates/accept` | Curation screen §3 `Accept families` (design handoff Q4). Body `{date, run_id?, note?}`; run defaults to the date's best run. Upserts one row per date into observations.db's **additive** `curation_accepts` table (`concert_date` PK, `run_id`, `accepted_at`, `n_judged`, `n_families`, `note`), created with `CREATE TABLE IF NOT EXISTS` on every call — the tapematch generator neither reads nor writes it, so this adds a curator surface without touching the schema the pipeline owns. `n_judged` is counted server-side from `pairs.human_judgment`, never taken from the client. Re-accepting replaces the row. 200 `{ok, date, run_id, accepted_at, n_judged, n_families}`; 400 `missing_fields`; 404 `no_run`; 409 `locked`. |
...
| ScreenTapeMatchCuration | `screens/ScreenTapeMatchCuration.tsx` | In-progress rebuild of the TapeMatch review workflow at `/tapematch/curation`, from `instructions/design_handoff_tapematch_curation/` (README §1–§12; build state + decisions in that folder's `WORK_PACKAGE.md`). **No nav entry** — per work-package D1 the screen is unreachable except by URL, and `ScreenTapeMatch` stays the shipped TapeMatch screen until this one reaches parity at phase 6. Phase 1 (2026-07-28, `0ee5f804`): `--lbb-fam-1..10` family identity colours in `lib/tokens.ts`, top bar, triage queue rail (j/k/enter/esc, needs/conflicts/all/done filters), date header with family chips and the verdict clamp, work grid + rail/dossier breakpoints. Phase 2 (2026-07-28): family-ordered similarity matrix off `/api/tapematch/pairs` — same-family cells tinted by family colour, non-family cells on an accent ramp, `n/c` hatching, LB-page conflict dot (`lb_says_same && !same_family`), symmetric selection with cross-dimming, `role="grid"` roving-tabindex navigation, and a compact 22px-column mode past 20 recordings. Phase 3 (2026-07-28): the §8 pair dossier, docked ≥1520px and a scrim-backed drawer below it (Esc/scrim/✕ close) — verdict block, conflict callout, A/B listening panel carried over from ScreenTapeMatch (96px reserved height, one-line reason when ineligible), four threshold-marked evidence bars (residual correlation, windowed coverage, quiet-segment hiss, demoted fingerprint dice with its 0.15–0.50 coincidence band), the LB-page claim quote, and the 2×2 judgment control with notes. The judgment control is UI only — its POST is phase 6. Phase 4 (2026-07-28): the §6 speed & lag strip off the new `/api/tapematch/sources` — every recording plotted on a signed-√ ppm axis (4–96% of the width) with ticks at min/`ref`/max, the DESIGN_ANSWERS A4 glyph vocabulary (◆ reference, ● aligned/constant offset, ▤ lag steps, ? speed-unknown incl. `insufficient`), greedy lane packing so clustered dots stack instead of overlapping, and dot clicks that build a pair two-at-a-time (work-package D7). Verdict cards are still a placeholder (phase 5). |

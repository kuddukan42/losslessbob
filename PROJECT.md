...
| Method | Route | Description |
|--------|-------|-------------|
| GET | `/api/gaps/grid` | **What ScreenGaps actually calls.** Whole-grid payload in one request: the same `{available, generated_at, totals, years}` shape as `/api/gaps/summary`, but each `years[]` entry also carries `dates: [{date_iso, coverage, label}]` — every cell the grid draws, `label` being the `venue or city` of that date's olof rows joined with `" / "`. Exists because the screen previously issued one `/api/gaps/year/<year>` per year (65 requests, each re-running the full-corpus coverage scan), which took ~12s to load; `gap_analysis.get_grid()` computes the coverage maps and event grouping once (BUG-275, 2026-07-24). |
...
| ScreenGaps | `screens/ScreenGaps.tsx` | Read-only "living Kokay list" at `/gaps` (Library nav group; 2026-07-17, TODO-256, `instructions/FABLE_GAPS_VIEW.md`). Decade-chip strip + year rows of small coverage cells (covered/partial/gap tones, future dimmed-outline) from a single `/api/gaps/grid` request (was `/api/gaps/summary` + one `/api/gaps/year/<year>` per year — 66 requests, ~12s to load; BUG-275). Clicking a cell opens a right-hand detail pane (single-tab tab-group — §6 will add a Family-tree tab) via `/api/gaps/date/<iso>`: olof event(s), circulating entries (LB deep-links into Library), month-partial candidates, TapeMatch families. Empty state when `olof_events` is absent. |

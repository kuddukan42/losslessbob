...
│   │   └── official_releases.json  # D-03 curated allowlist (TODO-342 C19, draft — tj sign-off pending): [{title, year, kind, official, patterns[]}], case-insensitive regexes; a string naming several releases takes the one named earliest (file order breaks ties); loaded by backend.dossier_fields.load_official_releases()
...
│   ├── taper_curation.html   # Taper curation workbench (served at /taper-curation) — all sources per entry side by side, isolated-text and per-taper tabs (TODO-327)
│   ├── qc_review.html        # QC review console (served at /qc-review, experimental) — Findings workbench + Queue tab (one-card, keyboard-driven, curator-gated writes) over qc_findings (TODO-342 C12)
...
│   ├── taper_curation.py     # Curation workbench read model (TODO-327): wide per-entry join across entries/lineage/attributions/TUIT/TapeMatch + isolated-text scans
...
│   ├── lb_coverage.py        # Coverage award (`design_handoff_lb_coverage_award`): read-only snapshot/coverage/stats rollup for GET /api/lb/coverage, plus get_ledger()/get_snapshots() behind /api/lb/coverage/ledger and /api/lb/snapshots (TODO-305); alias-folded held test shared with db.get_missing_from_collection (BUG-321)
│   ├── refresh.py            # Freshness planner (`instructions/PIPELINE_REFRESH_PHASE1.md` + `PHASE2.md`): 27-step registry + DAG, backlog/watermark signals read from existing computed_at/parsed_at/imported_at columns, `_parse_ts()` cross-format normalizer, publish-lag (D7); powers GET /api/refresh/status. TODO-306 P2: `_last_run_record()` merges `refresh_step_runs` into `last_run`/`last_run_source`, `version` block (`config_version.version_state()`) takes precedence over backlog in `_step_state()`, a newest `status='error'` run marks a step stale. TODO-310 P4: every step dict carries an `attention` list and the response carries `queues`/`queue_pending_total`, fed by a *lazy, guarded* import of `backend/queues.py` (dependency runs queues→refresh, never the reverse) — `attention` is orthogonal to `state`, which Phase 4 does not touch
│   ├── queues.py             # Pipeline refresh Phase 4 (`instructions/PIPELINE_REFRESH_PHASE4.md`): `RefreshQueue` registry of the four human review queues (taper conflicts, setlist-fingerprint suggestions, staged xref filesets, TapeMatch date curation) counted from the app DB only — never `tools/tapematch/observations.db`, which the nightly analysis runs hold locked. Two kinds: `gate` (expected to drain to zero; badged, produces step `attention` + chain advisories) and `backlog` (open-ended; reported as a ratio, never badged). `queue_counts()`/`attention_by_step()`/`pending_total()`/`snapshot()`; no new tables, counts derived on read; powers GET /api/refresh/queues
│   ├── refresh_exec.py       # Pipeline refresh Phase 3 (`instructions/PIPELINE_REFRESH_PHASE3.md`): `StepExecutor` registry mapping every `refresh.STEPS` step_id to an in-process call or a `JobState`-backed background worker; `plan_chain()` walks the DAG from a step_id/trigger seed pulling in stale ancestors (`_pull_stale_ancestors`), `run_chain_claimed()` runs the plan sequentially with a single-flight claim (`try_begin`/`finish`) and per-step `sub_progress`; TODO-310 P4 adds `advisories` to the plan (`_chain_advisories()` — one per pending `gate` queue blocking a runnable step, plus a `kind='publish'` entry when `master_publish` is runnable) which are display-only and never affect `/start` or the 409 path; all executor targets are resolved lazily inside wrapper functions so `concert_ranker`/`bs4`/`lxml` are never pulled into backend startup; 2026-09-21: `plan_chain(all_steps=True)` whole-pipeline scope (history `scope_kind='all'`) and `step_bucket()`/`annotate_buckets()` sort every step into ready/long/needs_you/fresh/untracked for the Updates card (a `ready` step is exactly what the all-scope chain runs); powers POST /api/refresh/chain/preview, /start, /stop, GET /status, /history
...
│   ├── dossier_fields.py     # Dossier field requirements (TODO-342 Phase 4 home): clean_track_title() (durations/suffixes/glued headers and "Encore N" stripped, slash-glued intros dropped, non-songs incl. "(encore break)" → None), parse_entry_tracklist() (per-track raw/title/is_song/partial/missing), match_track() (exact [spacing-insensitive, Talkin'→Talking folded] → no-subtitle → song_canonical alias → db.titles_match) + build_setlist_index(). C17: completeness(conn, event_id, lb_numbers) — D-01 per-source present/missing/partial setlist positions, basis tracklist/runtime/None, confidence corroborated/stated/inferred vs TUIT's per-LB present-count, show_bar; fits_show() + is_glued_tracklist() are the G2 source–show fit test, shared with QC rule R-E2. Accept: tools/dossier_acceptance.py --d01. C18: song_history(conn, event_id) — D-02 per-position tour_premiere / career_debut / last_played / gap_shows / times_played over concert-filtered song_performances in (date_str, event_id) order; three-part premiere gate (count = olof_events.tour_new_count, setlist.fm confirms each via corroborate.tour_premieres, no open R-O1/R-O4 on the tour up to this event) → premiere_badge; gap_badge at gap_shows ≥ GAP_BADGE_SHOWS (100) with no open R-O1 in the span. Accept + gate rate: --d02. C19: official_release(conn, event_id) — D-03 status full/partial/none from a whole-show `olof_events.releases_raw` line (no leading position-list digit) or per-position `olof_songs.released_on` coverage against `backend/assets/official_releases.json` (draft allowlist, tj sign-off pending) with `release_classifications` winning; `(part)`/`(uncertain)` tokens count toward partial, never full; `normalize_title_key()`, `classify_release_string()`, `split_release_tokens()`, `strip_release_prefix()`, `release_overrides()` are shared with rule R-R1 and the `/api/qc/releases/<title_key>` route. Accept: --d03. C20: run_context(conn, event_id) — D-07 tour object (Olof leg, NET #, position/size, is_first/is_last; "Recording sessions" year buckets suppressed), venue_run (consecutive concerts at one exact venue within the tour, nothing between), city_history ({year, venue, count} grouped by setlist.fm (city, country) with Olof's city as fallback; G5 sum invariant), claims_ok completeness guard (parsed songs, no open R-O1); rotation_rank(conn, event_id, venue_run) — D-04 rank / median / tie and superlative_ok via corroborate.rotation_check. Accept: --d07. C21: classify_generation(conn, lb) — D-05 silver / vinyl / broadcast / low_gen / master / unknown with basis stated/inferred and the firing evidence (a quarantined taper counts as no taper); classify_medium(conn, lb) — D-13 audio / audio_from_video plus a broadcast flag; media_available(); broadcast_tapers() feeds QC rule R-T5. Accept + audit table: --d05
│   ├── dossier_anchors.py    # Dossier view model (TODO-342 C25): Field {value, tier, source, confidence, derived_from}; ANCHORS registry (96 spec §6 anchors: tier, section, fallback kind, local_analysis flag); build_view(d1, conn, link_mode) → {fields: {key: Field}, rows: {prefix: [{sub: Field}]}} — every anchor present, derivation failures degrade to the fallback; filter_view() strips local_analysis anchors / unselected sections. Taper anchors via dossier_fields.taper_render(reload_aliases=False)
│   ├── dossier_claims.py     # Dossier claim engine (TODO-342 C26): Claim {kind, text_key, slots, scope, verified_by, ties, derived_from, local_analysis}; TEMPLATES; comparators superlative/exclusive/comparison + D-04/D-07/D-02 claim builders; attach_claims() adds view["claims"] and the verdict.why / context.chronicle slot templates
│   ├── dossier_qc.py         # Dossier QC gate (TODO-342 C28): run_gate() G1–G8 at the end of build_dossier → dossier["qc"] {checks_run, passed, withheld, refused, reasons, notices, input_fingerprint} + prov.withheld[]; rule → anchor quarantine maps; G1 refusal → /api/dossier/html + /bbcode 422; lint_l1 / lint_data_lb (G9)
│   ├── qc/corroborate.py     # Cross-source setlist corroboration (TODO-342 Phase 3a, audit Q1-a): setlist_quorum(conn, date_iso) compares Olof's primary-event setlist against setlistfm_setlist/bobdylan_setlist/tuit_song_performances aligned with the D-01 matcher (not by count); TUIT has no usable order (its `id` is alphabetical-by-song, verified on 1986-02-24), so it only ever corroborates on set+count; multi-show dates (early/late) pick the group best matching Olof; verdict 'disputed' (>=2 independent sources agree with each other against Olof) > 'corroborated' (>=1 agrees) > 'stated' (Olof has songs, neither holds) > 'unavailable' (nobody has a setlist — an empty Olof page against sources with data still runs the full comparison, so it reads 'disputed', not 'unavailable'); corroborate_all(conn) is the corpus-wide path (one preload query per source table) feeding R-O4; a local "Talkin'"→"Talking" fold handles the one D15 source-spelling gap the D-01 matcher's containment tier misses. C15 (Phase 3 rows (b)/(c), audit Q1-b/c, P10) adds three more on-the-fly checks, no new tables/rules: tour_premieres(conn, event_id) — per-song tour-premiere agreement between our corpus (song_performances joined olof_events, concert filter, (date_str, event_id) order) and setlist.fm, scoped to *our* tour's date span (_tour_start_date: the tour_name's earliest concert-filtered olof_events date through the day before this show — a C16 fix; setlist.fm's own tour_name buckets every NET-era show into one decades-wide "Never Ending Tour", which read real tour premieres like 2010-03-29's "Forever Young" as setlist.fm=False); 'ours'/'setlistfm' per position, 'setlistfm' is None when the source has no data for the date or doesn't list that song); verified live on 2010-03-29 (badges positions 4 and 14, premiere_count=2=tour_new_count, matching the D-02 accept case) in ~30ms (a DISTINCT on the earlier-tour-history query cuts ~36k NET rows to ~400 distinct titles). rotation_check(conn, event_id) — recomputes "songs not in the previous concert" from song_performances (previous concert = immediately preceding concert-filtered olof_events row by (date_str, event_id), not scoped to tour/venue) and compares to olof_events.rotation_new/rotation_pct (rotation_pct = floor(rotation_new/n_songs*100)); verified exactly on the 2010 Zepp Tokyo 7-night run (52,52,58,64,70,52,72) and agrees 87.3% corpus-wide (3068 stated events; close to the audit's cited ~86%, residual mostly 1974 shows where song_performances has far fewer rows than Olof's stated count). tracklist_check(conn, lb_number) — entries.setlist (dossier_fields.parse_entry_tracklist, song tracks only, missing excluded) vs tuit_recordings.setlist_json for the same lb_number, compared set+count (no order assertion) via the shared _compare helper; agrees 76.9% corpus-wide (3816 LBs with both sources live, vs the audit's ~3,881/82%-within-±2 figures — order-agnostic set+count agreement is a stricter measure than the ±2-length check); _parse_setlist_json_titles treats a valid-but-empty '[]' the same as no row at all (C16 fix — 294 LBs, 223 with an entries.setlist, previously read as "TUIT disagrees" instead of "TUIT has no tracklist"). rotation_corpus_agreement(conn) and tracklist_corpus_agreement(conn) are the corpus-wide paths (one preload pass each), surfaced by tools/dossier_acceptance.py --corroborate; --premieres runs the 2010-03-29 tour_premieres accept case. C16 (Phase 3 rows (d)/(e)/(f), audit Q1-d/e/f, D-11) adds three more, still no new tables/rules: file_format_check(conn, lb_number) — TUIT's lb_verified format ("FLAC 16/44") vs resolution figures parsed from entries.description's own unstructured "Lineage:" clause plus tuit_recordings.lineage (both read, concatenated, so a truncated side doesn't drop figures); parse_resolution_tokens(text) is the reusable regex (D-11/C22 will reuse it), whitelisted to real bit-depths/sample-rates only (a bare "N/M" scan without that whitelist matched date-like noise — "10/1", "11/0" — in live lineage text, a C16 fix); _recorded_and_final only reports a final_res when the text names a second, distinct (documented-conversion) figure, or when a lone recorder-only figure matches the file record — a lone figure that disagrees with the file is ambiguous (an undocumented conversion is plausible), not a false "disagrees" (C16 fix, found sampling live disagreements); agrees 99.3% corpus-wide (301/303), the 2 residual disagreements (LB-12156/12163) are a real bit-depth mismatch between a stated "convert to 24/44" and the actual 16/44 file. taper_check(conn, lb_number) — taper_attributions vs TUIT's taper field, through taper_agreement(ours, raws), the same verdict function R-T4 (rules.rule_t4) now calls, so the two can't drift (R-T4 open count unchanged at 67); corroborated 94.9% corpus-wide (1234/1301), the 67 disputes are genuine conflicting taper claims (curator review), not fold gaps. venue_check(conn, date_iso) — olof_events.venue/city vs setlistfm_shows.venue_name/city and bobdylan_shows.venue/location (split on its first comma for city), each folded (case/punctuation/accents/leading "The"; city also gets a small alias table for "New York City" vs "New York"); a multi-show date's source row is picked by _best_id_for_date, scored the same way the setlist checks pick among groups, but over every row the source has for the date (not just ones with a tracklist — an earlier version missed venue-only rows entirely); a source row that exists for the date but has a blank venue field doesn't count toward the verdict at all (C16 fix — was misread as "disputed" instead of "unavailable"); corroborated 82.9% corpus-wide (3362/4057), the disputes sampled are almost all genuine spelling/identity differences across sources (e.g. "Studio A" vs "Gerde's Folk City", "Montewese Hotel" vs "Montowese House") that per-spec exact-fold matching correctly declines to auto-merge. file_format_corpus_agreement(conn), taper_corpus_agreement(conn) and venue_corpus_agreement(conn) are the corpus-wide paths, surfaced by tools/dossier_acceptance.py --corroborate. Tests: tests/test_corroborate.py
...
│   ├── test_taper_curation.py # backend/taper_curation.py: cross-source agreement, filters, isolated-text populations (TODO-327)
...
│   ├── test_timeline.py      # backend/timeline.py: GRADE_RANK + summary/decade/tour rollup, three-state circulating (TODO-268)
│   ├── test_refresh.py       # backend/refresh.py: _step_state truth table (incl. backlog-beats-watermark false-positive guard), _parse_ts cross-format regression, every registry SQL executes against a real schema, DAG integrity/no cycles, missing-table → unknown; P4 additions pin the `attention` shape and assert **no step's `state` changes** when queues are unavailable
...
│   ├── test_dossier_anchors.py # backend/dossier_anchors.py: every anchor's fallback on a fresh DB, populated subset, local_analysis filter, registry key convention (TODO-342 C25)
│   ├── test_dossier_claims.py # backend/dossier_claims.py: ties, partial scope, disputed input → no claim, position/premiere guards, T4 templates, filter_view claims (TODO-342 C26)
│   ├── test_dossier_qc.py    # backend/dossier_qc.py: pass + fail per G1–G9, loose venue match, G7 scope drop, G8 refusal, 422 routes (TODO-342 C28)
│   ├── test_dossier_fields.py # backend/dossier_fields.py: clean_track_title, glued-marker split, partial/missing flags, match_track tiers (TODO-342 C13)
│   ├── test_qc.py            # backend/qc/: init_db idempotence, R-O1/R-O2/R-O3/R-T1/R-T2/R-T3/R-G1/R-E1/R-E2/R-F1/R-S1 fire/don't-fire, open→fixed→reopened lifecycle, false_positive stays/reopens on evidence change, quarantine lookup (TODO-342 Phase 2)
│   ├── test_official_release.py # backend.dossier_fields D-03 (official_release, normalize_title_key, classify_release_string) + qc.rules.rule_r1 (TODO-342 C19)
...
│   ├── dossier_acceptance.py # CLI: dossier-redesign live acceptance — one PASS/FAIL line per plan criterion (--parser; --before snapshot column; --corroborate; --premieres; --d01; --d02; --d03 D-03 official_release on 1965-06-01 full / 1986-02-24 partial / 1975-12-08 none) (TODO-342)
...
│   ├── wtrf.sh               # wrapper: wtrf_seed_board.py --delay 10; --limit 5000 unless --pages/--limit given (walks to the board's last page)
...
| event_type | TEXT | `'missing'` (watcher); `'content_changed'`, `'tags_changed'`, `'files_missing'`, `'restored'` (TODO-284 integrity scan) |
| detail | TEXT | Human-readable description |
| occurred_at | TIMESTAMP | Defaults to CURRENT_TIMESTAMP |
| acknowledged | INTEGER | 0 = unread, 1 = dismissed |
| mount_id | INTEGER | TODO-284: FK → `collection_mounts(id)`, set by integrity scans (NULL for watcher events) |
...
| status | TEXT NOT NULL | `'pending'`, `'downloaded'`, `'qbt_added'`, `'not_seeded'`, `'failed'` (default `'pending'`) |
...
### `lb_snapshot_history` — LB catalogue import history (USER table, TODO-305)
| Column | Type | Notes |
|--------|------|-------|
| id | INTEGER PK AUTOINCREMENT | |
| master_version / master_published_at | TEXT | Copied from the incoming manifest |
| imported_at | TIMESTAMP | ISO-8601 UTC stamp of the import |
| source | TEXT | `github` (installed release) or `file` (manual/local import) |
| entries_total / entries_held | INTEGER | `lb_master` rows after the import, and how many are in `my_collection` |
| entries_added | INTEGER | `entries_total` delta vs the previous history row (first import = its full size) |
| lb_status_changes | INTEGER | LBs whose `lb_status` moved in this import |
| status_counts_json / row_counts_json | TEXT | Post-import status histogram; rows written per master table |
| backup_path | TEXT | The `pre_master_import` backup taken before the swap |

Index: `idx_lb_snapshot_hist_at ON lb_snapshot_history(imported_at DESC)`. Written by
`db.import_master_db()` (best-effort — a bookkeeping failure never fails the import);
`meta` only holds the *current* `master_version`, so this is the only per-update diff.
Feeds `GET /api/lb/snapshots` and the `/lbdir/sync` screen.
...
### `tapematch_date_curation` — Curator sign-off per date (USER table)
Written by `POST /api/tapematch/dates/accept` (the curation screen's §3 `Accept families`),
read back as the `curated` state on `GET /api/tapematch/dates` — the triage rail's fourth
status, which wins over conflict/review because it is the curator's own terminal verdict.
One row per date, replaced on re-accept. Never exported in master data.

Deliberately in the app DB and not in `tools/tapematch/observations.db` alongside the
pair-level `human_judgment` it summarises: nothing in the tapematch pipeline reads an accept,
and observations.db is write-locked for hours by the nightly analysis runs, so an accept
stored there could fail purely because a batch was mid-flight. The route still *reads*
observations.db (read-only) to resolve the run and take the two provenance counts.
| Column | Type | Notes |
|--------|------|-------|
| concert_date | TEXT PK | YYYY-MM-DD |
| run_id | TEXT | Run accepted (a rerun can move it) |
| accepted_at | TEXT NOT NULL | ISO-8601 UTC |
| n_judged | INTEGER | Pairs with a `human_judgment` at accept time (provenance snapshot; the live count is always recomputed from the pairs) |
...
### `qc_findings` / `qc_decision_log` / `qc_runs` / `release_classifications` / `corrections` — Show Dossier QC (TODO-342 Phase 2, USER-tier, local-only)
Spec: `instructions/SHOW_DOSSIER_REDESIGN_PLAN.md` "Phase 2" (schema/rules) and "Decision
vocabulary" (status state machine). All five in `USER_TABLES`, never exported. `qc_findings`
holds one row per `(rule_id, entity_kind, entity_key)` (UNIQUE) — severity `error|warn|info`,
`detail`, `evidence_json`/`evidence_hash` (sha256 of canonical JSON, used to detect whether a
curator dismissal has gone stale), `first_seen`/`last_seen` UTC ISO, and
`status` `open|confirmed|false_positive|corrected|fixed` (index on `status, severity`).
`qc_decision_log` is an append-only audit trail mirroring `taper_decision_log`, written on every
status transition. `qc_runs` is one row per rule invocation (`n_open`/`n_new`/`n_fixed`/
`n_reopened`) for the `/qc-review` trend lines. `release_classifications` holds curator verdicts
on release titles (`title_key` PK, `official`), read by R-R1/D-03 alongside the asset allowlist
(this table wins). `corrections` holds curator field overrides (`entity_kind`, `entity_key`,
...
the standard letter-grade set mirroring `backend.timeline._GRADE_ORDER`), `R-E2` (error,
entity `entry`: an entry's tracklist doesn't fit its dated show — `entries.setlist` parsed by
`backend/dossier_fields.parse_entry_tracklist`, `date_str` mapped via `entry_date_to_iso`,
song tracks (non-songs and `missing` tracks excluded) matched by `match_track` against the
union of every Olof event's songs on that date; fires when <20% (`E2_MIN_SHARE`) of the
entry's song tracks match; skips entries with <3 song tracks (`E2_MIN_SONG_TRACKS`), dates
...
(table-level) — each check no-ops if either side's table is missing or empty, and `R-R1` (warn,
entity `release` keyed by `backend.dossier_fields.normalize_title_key`: a raw Olof release
string — a whole-show `olof_events.releases_raw` line with no leading position-list digit, or
an `olof_songs.released_on` token, `(part)`/`(uncertain)` prefix stripped — matches neither
`backend/assets/official_releases.json` nor a `release_classifications` override; evidence
`{raw, count, dates}` groups trivial spelling/whitespace variants of one string into a single
finding; live corpus 2026-09-11: 659 open); `store.py`:
...
### `collection_integrity_status` — TODO-284: latest per-LB integrity scan result
| Column | Type | Notes |
|--------|------|-------|
...
### `collection_integrity_scans` — TODO-284: integrity scan run history
| Column | Type | Notes |
|--------|------|-------|
| id | INTEGER PK | Auto-increment |
...
| GET | `/api/lb/coverage/ledger` | `backend.lb_coverage.get_ledger()` (TODO-305) — one page of the per-entry LB ledger behind `/lbdir/ledger`. Query: `page`, `per_page` (clamped to 500), `filter` (`all`/`held`/`missing`/`unmatched`/`review`; unknown values fall back to `all`), `q` (LB#/date/location), `lb` (deep link — returns the page holding that LB, ignoring `q`/`page`). Rows carry `lb_number`, `lb_status`, `date_str`, `location`, `fam_id`, `folder_name`, `filed_at`, `verified`, `needs_review`, `held` and a derived `state` (`verified`/`held`/`unmatched`/`missing`). Shares `_held_sql()` and the `nonexistent`-only exclusion with `/api/lb/coverage`, so the ledger and the coverage figure can never disagree. Feature-detected per table; a fresh DB returns an empty page, never a 500. |
| GET | `/api/lb/snapshots` | `backend.lb_coverage.get_snapshots()` (TODO-305) — LB catalogue import history for `/lbdir/sync`, newest first (`?limit=`, clamped to 200). Returns `{snapshots[], current, total}` from `lb_snapshot_history`; a DB whose catalogue predates that table gets one `synthetic: true` row derived from `meta` + live coverage instead of an empty page. |
| GET | `/api/refresh/status` | `backend.refresh.compute_plan()` — pipeline-freshness snapshot (`instructions/PIPELINE_REFRESH_PHASE1.md` + `PHASE2.md`). Returns `steps[]` (`step_id`, `trigger` T1–T4, `kind`, `state` `fresh`/`stale`/`blocked`/`unknown`, `reason`, `last_run`, `last_run_source` `run_record`/`watermark`/`None`, `last_run_status`, `age_days`, `backlog`, `blocked_by`, `upstream`, `how_to_run`, `cost`, `human_gate`, `version` `{key, state, expected, stored}`), `stale_count`/`blocked_count`/`unknown_count`, `by_trigger`, and `publish_lag` (the D7 fix: `days_since` + `lb_status_changes_since` from `lb_status_history` + `entries_scraped_since`). Optional `?trigger=T1` filter narrows `steps` and all counts. State precedence (TODO-306 P2): `version.state=='changed'` → stale (even at backlog 0) → **backlog** count where computable → newest `refresh_step_runs` row `status='error'` with no later success → stale → upstream blocked/stale → **watermark** comparison (only when no backlog signal exists, so `backlog=0` is never overridden) → `unknown`/`fresh`; `version.state=='unstamped'` never downgrades state, only annotates `reason`. `last_run` is `max(watermark, newest successful refresh_step_runs row)`. All timestamps are normalized through `_parse_ts()` before comparison and no timestamp is ever compared in SQL across tables. GET is read-only, no curator gate; a missing table degrades that step to `unknown` rather than raising. TODO-310 P4: every step also carries `attention[]` (`{queue_id, count, kind}`, empty for the 23 steps no queue names) and the response carries `queues[]` + `queue_pending_total` (gate queues only). `attention` never affects `state`. |
| GET | `/api/refresh/queues` | `backend.queues.snapshot()` (`instructions/PIPELINE_REFRESH_PHASE4.md`). Returns `{queues[], pending_total, computed_at}`; each queue is `{queue_id, label, kind gate/backlog, count, total, blocks[], screen, action, state pending/open/clear/unknown}`. Four sub-millisecond counts — the standalone route exists so the sidebar badge can poll without recomputing the 27-step plan. `state='unknown'` when the table is absent; `pending_total` sums `gate` queues only. Read-only, no curator gate. |
...
| POST | `/api/olof/parse` | Body `{file?}`. TODO-306 P3: synchronous reparse of local DSN pages (`corpus='dsn'`) via `backend.olof_parser.run_parse()`; 409 while the olof fetch job is running; 503 if `backend.olof_parser` isn't importable (bs4/lxml missing). Records a `refresh_step_runs` row (`olof_parse`). |
| POST | `/api/olof/chronicle_parse` | Body `{file?}`. TODO-309: same shape as `/api/olof/parse` for the chronicle corpus, via `backend.olof_chronicle_parser.run_parse()` — a separate module owns those pages, and `olof_parse` selects `corpus='dsn'` only. Records a `refresh_step_runs` row (`olof_chronicle_parse`). |
...
| POST | `/api/refresh/chain/preview` | Body `{step_id?, trigger?, all?, include_expensive?}` (exactly one of `step_id`/`trigger`/`all`; `all` = every stale/blocked step, the Updates card's single button). `backend.refresh_exec.plan_chain()` — read-only DAG walk from the seed pulling in stale ancestors; claims nothing, starts nothing, safe to call on every keystroke. 400 on a bad seed. TODO-310 P4: also returns `advisories[]` (`{queue_id, count, step_id, kind queue/publish}`) for the preview dialog — advisory text only, never a gate. |
...
| GET | `/api/tapematch/pairs?date=` | One date's `tapematch_pairs` rows for the similarity matrix: `{date, run_id, pairs: [{lb_a, lb_b, corr, emb_score, fp_score, same_family, similarity_pct, human_judgment, human_notes, ab_eligible, lb_says_same, lb_relation_text, windowed_frac, hiss_median}]}`. `human_*`, `ab_eligible` (TODO-231/233), `lb_says_same`/`lb_relation_text` and `windowed_frac`/`hiss_median` read LIVE (best-effort) from observations.db; null on lock/missing. Neither the LB-page claim nor the two secondary-evidence metrics are in `tapematch_pairs` — they ride the same enrichment read so ScreenTapeMatchCuration's matrix can mark conflicts (`lb_says_same && !same_family`) and its dossier can draw the §8 evidence bars. All six enriched columns post-date observations.db's first schema, so they are probed with `PRAGMA table_info` — an older DB nulls exactly the columns it lacks rather than raising and collapsing the whole enrichment (`ab_eligible` included). `ab_eligible` is resolved per pair via `ab_clips.get_pair_source_info` (each pair's own latest common tapematch run) + the speed-kind and post-2026-07-06 run gates, matching what `POST /api/ab_clip` will accept — not the (possibly stale) run_id synced into `tapematch_pairs`. 200 with `pairs: []` for un-synced dates. |
...
| GET | `/api/tapematch/sources?date=` | Per-recording speed/lag facts for the curation screen's §6 speed & lag strip: `{date, run_id, sources: [{lb_number, speed_kind, speed_ppm, family_id, folder_name, lag_ref_lb}]}`, read live from observations.db's `sources`. Source-shaped rather than pair-shaped, hence its own route — and the only tapematch view with something to show on a single-recording date. Rows come from the LATEST run holding sources for the date (same rule as `ab_clips.get_source_info`), not the possibly-stale run_id synced into `tapematch_pairs`, so a rerun's new `speed_kind` shows immediately. Columns probed with `PRAGMA table_info`; a missing/locked DB or an unanalysed date returns `sources: []` with a null run_id rather than an error. |
| GET | `/api/tapematch/report?date=` | Best run's `report.md` for a date (curation screen §11): `{date, run_id, run_dir, report_md\|null}`. Same run resolution and read-only disk read as `/api/tapematch/analysis` — the two documents are siblings in the run's archive dir — plus `run_dir`, which the sheet header prints so a curator can find the file on disk. `report_md` null (run_id/run_dir still populated) when the run predates the report writer; all four null when the date has no run. 409 `locked`. |
| GET | `/api/tapematch/runs?date=` | Every run recorded for a date, newest first (curation §12's two run pickers — 1989-06-04 has 17): `{date, runs: [{run_id, run_at, n_sources_db, n_sources_found, n_sources_ran, n_families, duration_sec}]}`, straight off observations.db's `runs` table with the optional columns probed. `runs: []` for an unknown date or an absent/locked DB. |
| GET | `/api/tapematch/run_snapshot?date=&run_id=` | One named run's own `sources` + `pairs` rows for the §12 diff: `{date, run_id, run: {...}|null, sources: [{lb_number, family_id, folder_name, speed_kind, speed_ppm}], pairs: [{lb_a, lb_b, corr, emb_score, windowed_frac, hiss_median, fp_score, family_id_a, family_id_b, tapematch_verdict, human_judgment}]}`. The named run, never the latest — comparing two runs is the whole point. The diff itself is computed in the renderer (README §12: neither run is mutated by viewing). Unknown run → `run` null + empty lists; 400 `missing_fields`. |
| GET | `/api/tapematch/dates` | Left-rail summary, date DESC: `{dates: [{date, run_id, n_lbs, n_pairs, has_analysis, needs_review, location, curated, curated_at}]}`. `location` resolved via the date's LB numbers (entries.date_str is US-format, never joined on). has/needs fields null when observations.db missing/locked. `curated`/`curated_at` come from the app DB's `tapematch_date_curation` (see the accept route below) — the curation screen's fourth triage state, which wins over conflict/review because it is the curator's own terminal verdict, and which survives a locked observations.db unlike the has/needs fields. |
| POST | `/api/tapematch/dates/accept` | Curation screen §3 `Accept families` (design handoff Q4). Body `{date, run_id?, note?}`; run defaults to the date's best run. Upserts one row per date into the app DB's `tapematch_date_curation` (USER table) — **not** observations.db, because nothing in the tapematch pipeline reads an accept and a nightly batch holding that DB must not be able to block one. observations.db is still read (read-only) to resolve the run, count `pairs.human_judgment` server-side for `n_judged` (never taken from the client), and copy `runs.n_families`. Re-accepting replaces the row. 200 `{ok, date, run_id, accepted_at, n_judged, n_families}`; 400 `missing_fields`; 404 `no_run`; 409 `locked` (only from the observations.db read). |
...
### Show Detail (`backend/gap_analysis.py`; formerly TODO-256's Gaps View, `instructions/complete/FABLE_GAPS_VIEW.md`)
| Method | Route | Description |
|--------|-------|-------------|
| GET | `/api/shows/<iso>/olof` | Drill-down for one date: full `olof_events` rows, exact-date `entries` (lb_number/rating/status/taper_name), month-partial candidate entries, `recording_families` on that date if the table exists. Backs the Library DetailPanel's Olof tab for `uncirculated`/`upcoming` (recording-less) performance rows. Renamed off its original route when the standalone Gaps screen was retired — its coverage grid became the Library performance lens's `coverage` field/`uncirculated` view/Coverage facet, fed by `gap_analysis.uncirculated_dates()` via `db.get_performances`. |
| GET | `/api/timeline/summary` | Decade-by-decade rollup, computed live (no derived table — `backend/timeline.py`). Concert-only `olof_events` grouped by `date_str`, best letter-grade per decade via `GRADE_RANK`. Returns `{available, generated_at, decades: [{decade, label, night_count, circulating_count, best_grade}]}`; `available: false` (HTTP 200) when `olof_events` is absent. |
| GET | `/api/timeline/decade/<int:decade>` | Tours whose earliest show falls in that decade: `{available, decade, label, tours: [{tour_name, start_date, end_date, night_count, circulating_count, best_grade}]}`, sorted by start date. |
| GET | `/api/timeline/tour` | `?name=<tour_name>&decade=<int>` (query params — tour names carry spaces, aren't globally unique). Night-by-night: `{available, tour_name, decade, nights: [{date_iso, venue, city, best_grade, circulating}]}` — three states: graded (best_grade set), held-but-ungraded (best_grade null, circulating true → dossier exists), no-tape (circulating false). |

### Show Dossier (TODO-257, `instructions/FABLE_SHOW_DOSSIER.md`)
| Method | Route | Description |
|--------|-------|-------------|
| GET | `/api/dossier?date=&location?=&channel?=` | `backend.dossier.build_dossier()` — everything the app knows about one date: show identity, historical context (`olof_chronicle`/bobtalk/notes/lineup), rarity-flagged setlist (`song_performances` all-time counts; `only`/`first`/`last`/`rare` at ≤10 performances), sources grouped by `recording_families`/`tapematch_family_meta` with taper credit (`taper_attributions`, conflicted rows skipped), lineage notes (`entry_lineage`), pick ranking + evidence (`show_picks`), quality verdicts (`quality_recording_scores`), curated-list endorsements, alt-fileset counts (`checksums` xref), and a rank-1 `recommendation`, plus (TODO-342 C25) an additive `view` — the anchor view model from `backend.dossier_anchors.build_view()` (absent only if view assembly raised, which is logged). Every section is feature-detected and omitted (never null-faked) when its source table is absent or empty — a fresh install still returns a valid, smaller dossier. Ambiguity is detected via distinct `olof_events.venue` values for the date (not `entries.location`, which is noisy free text with many spellings of the same real venue) — HTTP 300 with `{ambiguous, date_iso, candidates}` when more than one exists and `location` wasn't given. `channel='public'` (default) reduces any `entries.status='private'` source to `{lb, private: true}` only; `channel='full'` includes everything. Disk paths, collection ownership, friend data and wishlists are never touched. |
...
| GET | `/api/tapers/curation?state=&confidence=&conflict=&taper=&q=&attributed=&tuit=&family=&agreement=&limit=&offset=&sort=` | One page of the `/taper-curation` workbench: per entry, the LB text + full description, the `entry_lineage` parse, the attribution + evidence, the curator decision, the TUIT recording (`null` = never scraped), the TapeMatch family with `best_similarity`/`pair_count`, the known-alias hits found in the text, and the derived `agreement` verdict (agree/conflict/tuit_only/lb_only/attr_only/none). `confidence=none` selects unattributed rows; `tuit` is scraped/not_scraped/has_taper/taper_only; `family` is has/none/review. `agreement` is computed, so it resolves over a cheap projection first and only the page's LBs are re-selected in full. 400 on a bad enum/sort. Open (no curator gate). |
| GET | `/api/tapers/curation/isolated?kind=&q=&limit=&refresh=1` | The two texts-that-never-became-a-credit populations, grouped by canonical with variants, entry counts and sample LBs. `excluded` = resolves to a vocabulary-barred canonical (`_NOT_TAPER` builtins, `user_taper_flags`) — only recoverable from raw description text, since the parser drops those names before `entry_lineage`, so it is a full-corpus regex scan (~12s) cached on an entry-count/vocabulary fingerprint; `refresh=1` forces a rescan. `unknown` = normalises to nothing the vocabulary knows. Open. |
| GET | `/api/tapers/curation/tapers` | Per-canonical rollup across attribution tiers, curator decisions and TUIT tag counts, ordered by undecided count. Open. |
| GET | `/api/tapers/curation/text/<lb>?needle=` | Up to five description snippets around each occurrence of `needle` in one entry. 400 without a needle. Open. |
| GET | `/api/qc/summary` | TODO-342 Phase 2b (`backend/qc/review.py`). Per rule: description, default severity, counts by severity × status, last `qc_runs` row; plus total open (open+confirmed) errors and warnings and the last run time. Open. |
| GET | `/api/qc/findings?rule=&severity=&status=&entity_kind=&date=&q=&page=&page_size=` | One page of `qc_findings`, newest `last_seen` first, each row with `affected_dates` (ISO dossier dates the entity touches) and `shows_affected`. `date` filters on affected dates (computed, so resolved before paging); `page_size` ≤ 200. Open. |
| GET | `/api/qc/findings/<id>` | One finding: parsed evidence, rule description, `quarantined`, a rule-family `context` panel (taper attribution + entry + TUIT tapers · Olof raw text vs parsed songs · `venue_geocoded` row · entry row · family meta + members · staleness evidence) and `affected_dates`. 404 when missing. Open. |
| GET | `/api/qc/decisions?page=&page_size=&finding_id=` | `qc_decision_log`, newest first, optionally one finding's history. Open. |
| POST | `/api/qc/findings/<id>/decision` | TODO-342 Phase 2b C12 (`backend/qc/decisions.py`), curator-only. Body `{status: 'confirmed'\|'false_positive', note?}`. Legal from `open`/`confirmed`/`false_positive`; `fixed`/`corrected` → 409. `false_positive` binds to the finding's current `evidence_hash` — `qc/store.run_rule` reopens it if the evidence changes on a later run. 404 unknown id, 400 bad status. |
| POST | `/api/qc/findings/bulk` | Curator-only. Body `{ids: [int], status: 'confirmed'\|'false_positive', note?}` — one rule per batch (400 on a mixed-rule batch, nothing written), all-or-nothing. |
| POST | `/api/qc/corrections` | Curator-only. Body `{finding_id, field, corrected, reason?, original?}` — inserts a `corrections` row and moves the finding to `corrected`. 404 unknown finding, 409 illegal transition. |
| POST | `/api/qc/releases/<title_key>` | TODO-342 C19 (`backend/qc/decisions.py` `classify_release`), curator-only. Body `{official: bool, note?}` — upserts `release_classifications` (wins over `backend/assets/official_releases.json`) and immediately closes (`fixed`) any open/confirmed/false_positive `R-R1` finding sharing that `title_key`; 400 on a non-bool `official`. |
| POST · GET | `/api/qc/run` | Curator-only POST starts a rules run (`backend/qc/jobs.py`, `JobState`-backed like `backend/ranker_jobs.py`), body `{rule_id?}` for one rule or all; 409 if already active, 400 unknown rule_id. GET (open) polls progress/result. |
...
| GET | `/api/collection/export/html` | Download My Collection as a self-contained HTML table. Optional `?cols=` (comma-separated, ordered) picks which columns to render/export — `lb` always included; see `_EXPORT_COLUMN_DEFS` in `app.py` for the full set (base six plus `disk_path`/`confirmed_at`/`source_type`/`lb_category`/`rating`). Falls back to the base six if omitted/invalid. Optional `?lb_numbers=1,2,3` restricts the export to a subset (the GUI passes its currently filtered/searched rows). Returns `collection.html` attachment. |
| GET | `/api/collection/export/html/missing` | Download the "Not in collection" LB list (`database.get_missing_from_collection()`) via the same interactive HTML report, with a column set for never-owned entries (`_EXPORT_MISSING_COLUMN_DEFS`: lb/status/date/location/rating/description). Optional `?lb_numbers=1,2,3` restricts the export. Returns `missing_lbs.html` attachment. |
...
| GET | `/api/entry/<lb>/lbdir_status` | LBDIR verdict for the LB's filed collection folder (shown in the forum post preview). Audio-first: `audio_status` is green when every audio entry passes, with the text entries described separately (`text_label_key`). Missing non-audio entries are auto-reconciled first (exact-MD5 in-folder moves + site-mirror copies; never audio, never deletes). Reuses the cached `pipeline_folder_state` verdict while the folder fingerprint matches; `?force=1` re-verifies. Returns `{status, label_key, label_params, check, audio_status, audio_label_key, audio_label_params, text_label_key, text_label_params, split, reconciled, folder, manifest, cached, checked_at}`. |
...
| GET | `/taper-curation` | Serve the taper curation workbench (`backend/taper_curation.html`) — self-contained, no build step, opened directly in a browser. **Workbench**: a dense grid over all 16.7k entries (LB / date+place / attribution / TUIT / LB text / match / state) beside a detail pane carrying the full source comparison, the LB description with every known alias highlighted by vocabulary verdict, the TUIT info file, and the decide row; `j`/`k` move, `c` confirms the attribution, `t` confirms the TUIT tag, `r` rejects, `u` marks unresolved, `x` multi-selects for bulk. Presets cover the queues that matter: TUIT taper with no attribution, sources disagree, engine conflicts, propagated + undecided. **Isolated text**: the excluded/unknown groups with add-as-taper, alias-of and not-a-taper actions. **Tapers**: the per-canonical rollup. Reads are ungated; writes reuse the curator-gated confirm/reject/unresolved, bulk and vocabulary routes, so decisions land in `taper_decision_log` exactly as `/taper-review` records them (TODO-327). |
| GET | `/qc-review` | Serve the QC review console (`backend/qc_review.html`, TODO-342 Phase 2b, experimental, English-only) — self-contained like `/taper-curation`. Header: open errors · open warnings · last run · **Run rules** (posts `/api/qc/run`, polls to completion). **Queue** (default tab): one open finding at a time (error before warn, then rule_id, then id; optional rule filter), rule-family evidence panel (taper mention highlight + Reject attribution + Open in /taper-review for R-T; Olof raw vs parsed for R-O; geocoder match for R-G1; raw release string + Official/Not official buttons calling
`/api/qc/releases/<title_key>` for R-R1; raw row dumps for R-E1/R-F1/R-S1), affected-shows Open-dossier links, decision row (Confirm/False positive/Correct…/Skip + note), keys `c f e s j k o`. Current id + skipped ids persist to `localStorage` (try/catch-wrapped) so a reload resumes. **Findings**: `qc_findings` workbench with rule/severity/status chips, search, pager, detail drawer, and multi-select bulk Confirm/False-positive (one rule per batch, else disabled). A curator pill reflects `/api/curator`; any 403 surfaces "curator mode is off", not a silent failure. |
...
| POST | `/api/lbdir/reconcile` | Preview-only: scan disk files recursively, compute MD5, match against missing lbdir entries. Returns `{results: [{folder, proposals:[{disk_rel,lbdir_rel,md5,expected_md5,matched_by}], unmatched_lbdir, unmatched_disk, warnings, site_proposals}]}`. Does NOT move any files. Both `proposals` and `site_proposals` carry `matched_by`: `'md5'` (exact content match) or `'name'` (filename matches after stripping `LBF-{N:05d}-`, but `md5` != `expected_md5` — the copy is a different revision, BUG-252). Unreconcilable entries (self-referencing `lbdir-*.txt` manifests, regenerated DigiFlawFinder reports) are excluded from missing sets and proposals entirely. `site_proposals` lb_number resolves via `my_collection` -> folder name -> optional `lb_number_hint` body param. |
...
| Screen | File | Status |
|--------|------|--------|
| ScreenHome | `screens/ScreenHome.tsx` | Done — dashboard, live stats, activity log, flat-file update; first-run onboarding (TODO-217): auto-opens `components/OnboardingWizard.tsx` (4-step modal: master install → sitedata install → mounts/pipeline nav → done + `/api/derived/recompute`) once per launch when `entries_count == 0`, plus a setup-checklist card while `/api/onboarding/status` `complete == false`; `components/DataFreshnessCard.tsx` is the **Updates** card (redesigned 2026-09-21; design in Claude Design project "LosslessBob — Updates card redesign"): steps are grouped by who does the work, using the server-side `bucket` from `/api/refresh/status` — **Ready to update** (numbered in `update_order`, why/source/time-word columns; one primary "Update N steps" button opens a confirm dialog fed by `/api/refresh/chain/preview {all:true}` with queue advisories, and starts `/chain/start {all:true}`; while running the card shows a progress bar + per-step ✓/› list and a "Stop after this step" action), **Long jobs** (cost `very_slow` job steps: `scrape_entries` via `/api/scrape/*`, `ranker_scan` via `/api/ranker/scan` with "Scan N new" / "Rescan all…" confirm; never part of Update), and **Needs you** (pending gate queues + manual steps, one Open button to the owning screen, copyable command when no screen owns it; publish lag shows as the Publish master row's reason). Footer: backlog-queue ratio bars and a collapsed "N up to date · M not tracked" list. Trigger groups, Stale/Blocked chips, cost pills and the include-expensive checkbox are no longer shown. Hides itself entirely when the route is absent or errors. `AppShell.tsx` polls `/api/refresh/queues` every 60s and badges the Library and Fingerprint nav items via `lib/navigation.ts`'s `NAV_QUEUE_BADGES` (TapeMatch and xref are deliberately unbadged) |
...
| ScreenLibrary | `screens/ScreenLibrary.tsx`, `components/library/actions.tsx`, `components/library/DetailPanel.tsx` | In progress (TODO-150) — "By performance \| By recording" lens toggle (defaults to performance). Both lenses share the same merged `RecordingRow[]` (from `/api/search` + `/api/collection/prefetch` + `/api/library/badges` — the last merged by `lb_number` for pick/curated/taper badges, TODO-212); the performance lens additionally fetches `/api/library/performances` + `/api/tapematch/families` and groups recordings by show → TapeMatch family → member. The recording lens renders ★ recommended / curated / `absGrade` / confirmed-taper (upgrading the raw taper pill) badges inline; the performance lens's view menu includes a combined "Any curated pick" (`curatedAny`) filter alongside the per-curator ones. Each lens has its own facet rail and virtual year-grouped table. Right-click context menu and a checkbox bulk-select bar (recording lens: Create torrent/Add to qBittorrent/Update location/Remove) are wired in via `actions.tsx`'s shared registry. Selecting a row opens a zoned detail panel (`DetailPanel.tsx`: ActionBar/ShareSeed/AssetStrip/Setlist, step 8; recording lens also has a Quality tab showing LB Rating + Concert Ranker AI Quality Index side by side, a Picks tab (show-pick rank/score + `EvidenceList`, lazy `/api/picks/for/<lb>`), and a Taper tab (attribution tier/conflict + `EvidenceList`, lazy `/api/tapers/attributions/<lb>`, confirm/reject buttons in curator mode) as a third column on either lens; TODO-162 P5b added an Olof tab on both lenses (Still On The Road setlist with encore/credits/annotations/take status, NET + year concert #s, recording info, notes, BobTalk quote, chronicle diary entries, circulation provenance, and a per-copy setlist comparison via `POST /api/olof/compare`) — gated on `/api/olof/status` `events > 0` (react-query `staleTime: Infinity`), so it never renders on installs without local Olof data. Live at `/library`, sidebar nav item "Library" (featured, top of the Library group, above My Collection) — step 9. Performance lens show rows and the detail panel render an "Unconfirmed" pill for degraded (`confirmed: false`) shows recovered from the `lb_category='unknown'` bucket (TODO-151). `m3u` (Export show as M3U) is wired for performance rows. `sources`/`notify` row actions and the TapeMatch family `note` field remain unexposed (no backend/UI to wire to). i18n pass (step 10) **done** — all in-screen strings extracted to the `library` namespace in `locales/*.json` and the three files converted to `t()` (the shared action registry takes a `TFunction` param since the builders are plain functions; counts pluralised via `_one`/`_other` keys); the 5 non-English locales filled via DeepL. The performance lens absorbed the retired standalone Gaps screen: `db.get_performances` unions in recording-less rows for every `olof_events` concert date with no circulating recording (`gap_analysis.uncirculated_dates`), carrying an explicit `coverage: 'uncirculated'` (past, no tape) or `'upcoming'` (not yet happened) field; a matching `uncirculated` view and Coverage facet surface them, and the detail panel's Olof tab falls back to `GET /api/shows/<iso>/olof` (`OlofShowZone`) for these rows since they have no lbNumber to key the recording-aware Olof/compare fetches on. TODO-310 P4: a one-shot `?view=<PerfView>` search param (same consume-then-clear shape as `?lb=`) switches to the performance lens and applies that view — this is what the Data Freshness card's taper-conflict "Review →" button targets (`/library?view=taperReview`). |
| ScreenBootlegs | `screens/ScreenBootlegs.tsx` | Done — year/CDs filters, catalog browser, CSV export |
| ScreenTapeMatchCuration | `screens/ScreenTapeMatchCuration.tsx` | **The** TapeMatch screen at `/tapematch` (Library nav group) since 2026-07-28, built from `instructions/complete/design_handoff_tapematch_curation/` (README §1–§12; build state + decisions in that folder's `WORK_PACKAGE.md`). It replaced the read-only `ScreenTapeMatch.tsx` (deleted) once the phase-6 write path reached parity; `/tapematch/curation` redirects to `/tapematch`, query string preserved. §1 top bar (crawl status + start/stop, carried over from the old screen), §2 triage queue rail (j/k/enter/esc, needs/conflicts/all/done filters), §3 date header with family chips, verdict clamp and `Accept families`, §5 family-ordered similarity matrix off `/api/tapematch/pairs` (same-family cells tinted by family colour, non-family cells on an accent ramp, `n/c` hatching, LB-page conflict dot, symmetric selection with cross-dimming, `role="grid"` roving-tabindex nav, compact 22px columns past 20 recordings), §6 speed & lag strip off `/api/tapematch/sources` (signed-√ ppm axis, A4 glyph vocabulary, greedy lane packing, two-click pair building per D7), §7 analysis verdict cards parsed client-side by `lib/analysisMd.ts` with the raw `analysis.md` behind a disclosure (also carried over), and the §8 pair dossier — docked ≥1520px, scrim-backed drawer below — with verdict block, conflict callout, A/B listening panel, four threshold-marked evidence bars, the LB-page claim quote, LB headings deep-linking to `/library?lb=`, and the judgment control writing `human_judgment`/`human_notes` via `POST /api/tapematch/pairs/judgment` (explicit Cancel/Save per D4, §10.7 save-status line). `Accept families` posts to `/api/tapematch/dates/accept` → `tapematch_date_curation`, which is what makes a date `curated`. §11 `report.md` overlay (phase 7): `Open report.md` in the date header opens `components/tapematch/ReportSheet.tsx`, a portalled read-only sheet over the workspace off `/api/tapematch/report`, parsed by `lib/reportMd.ts` — Coverage as a stat row with warn-barred not-on-disk rows (A3), the `## tapematch output` fence split into one collapsible x-scrolling monospace panel per `=== MARKER ===` with DIAGNOSTICS/CLUSTERS open by default (A1), an outline rail nesting those markers with counts only where a count means something (A2), LB chips and clickable audit rows that select a recording or open a pair's dossier, a stale banner plus dashed `YOUR JUDGMENT` annotations wherever a judgment post-dates the file, a Rendered/Raw toggle (line gutter + four-token tinting), Copy/Download/Print, and a §11.1 print block that re-inks the document for paper. Prose blocks render through `react-markdown` + `remark-gfm` (Q5). §10's edge and transient states (phase 9): skeletons that render the real grid so nothing reflows, a fetch-error block carrying the real request/error/run/attempt, per-filter rail empty copy, mute zero-recording and solo-recording states, and the dossier drawer's slide-in + focus trap. §12's run diff (phase 8): `Compare runs` in the date header opens `components/tapematch/RunDiffSheet.tsx` — the same `SheetShell` overlay as §11 — with two run pickers off `/api/tapematch/runs`, two snapshots off `/api/tapematch/run_snapshot`, and the diff itself computed in `lib/runDiff.ts` (successor-mapped family regrouping, a delta matrix whose fill is magnitude and whose ring is a flipped call, a pair-change table and a judgment-impact list). §12.1's pipeline-cause list is stated as underivable rather than guessed: runs record no threshold set. The build is complete — all nine phases shipped. **Not internationalised** — all strings are hardcoded English (TODO-275); the retired screen's `tapematch.*` locale keys are kept for that pass. |
...
| ScreenTimeline | `screens/ScreenTimeline.tsx` | Read-only timeline navigator at `/timeline` (Library nav group; 2026-07-24, TODO-268, `instructions/complete/FABLE_TIMELINE.md`). Zoomable Decade→Tour→Night grid with breadcrumb back-out, cells colored by best grade on the sequential `--lbb-seq-*` ramp (`tokens.ts`). Three visual states per tier — graded (ramp), circulating-ungraded (neutral held-fill), no-tape (`--lbb-mute-*`). Fed by `/api/timeline/summary` → `/decade/<int>` → `/tour?name=&decade=`. Clicking a graded or held-but-ungraded night opens an in-app dossier iframe viewer (`/api/dossier/html?...&inline=1`) with an "Open full export…" hand-off to `DossierExportModal`. Empty state when `olof_events` is absent. |
...
| ScreenChecksumDisputes | `screens/ScreenChecksumDisputes.tsx` | Curator triage of checksum_disputes at `/checksumdisputes` (Curator nav group, gated; TODO-299). Reads `GET /api/checksum-disputes?grouped=1` — findings pair the db/lbdir rows for one disputed value, bucketed db_error / audio_differs / retag / receipt_unknown / lbdir_only (`checksum_provenance.get_findings`); verdict + reference filters; confirm/dismiss via `PUT /api/checksum-disputes/<id>`. Library detail panel shows an open-dispute badge; Lookup flags NOT FOUND rows the uploader's checksum vouches for. |
| ScreenCoverage | `screens/ScreenCoverage.tsx` | "Complete against LB" progress/award screen at `/about/coverage` (2026-08-12, `instructions/design_handoff_lb_coverage_award`). Not a sidebar destination — reached from the About dialog's "Collection progress" row, but a real deep-linkable route so the milestone survives a reload. One component; the gold award treatment is a state of it (`coverage.complete`), not a separate screen. Fed by `/api/lb/coverage`; per-decade bars, catalogue-snapshot provenance, and a client-side 2× canvas certificate renderer + export modal (PNG). Award gold is the fixed per-mode `--lbb-award-*` ramp in `tokens.ts`, never an accent, so it reads identically under all eight accents and both frame palettes. |
| ScreenLbdirLedger | `screens/ScreenLbdirLedger.tsx` | Full per-entry LB ledger at `/lbdir/ledger` (2026-08-17, TODO-305). Reached from ScreenCoverage's "View full ledger"; not a sidebar destination. Fed by `/api/lb/coverage/ledger`: filter chips (all / in collection / missing / no family yet / needs review), free-text search over LB#/date/location, 100-row paging, per-row state pill (verified/held/unmatched/missing) with matching edge tone; a row click opens `/library?lb=<n>`. `filter`/`page`/`q` are URL state so a view is shareable and survives a reload; `?lb=<n>` deep-links to the page holding that entry (backend resolves the page, then the URL is rewritten to it). |
| ScreenLbdirSync | `screens/ScreenLbdirSync.tsx` | LB catalogue sync at `/lbdir/sync` (2026-08-17, TODO-305). Installed-catalogue header (snapshot label/version/published/imported/entries), a manual "Check LB for an update" (`GET /api/master/github_check` — the install itself still happens in Setup, which the newer-version banner links to), and the import history table from `/api/lb/snapshots` with per-update entry delta and status-change count. Rows predating `lb_snapshot_history` are shown as one muted `synthetic` row labelled as such. |
...
- **State: zustand stores, not Context/Redux.** `store.ts` holds app-wide settings (`useSettingsStore`, persisted to localStorage via `zustand/middleware`'s `persist`). Screen-scoped shared state (folder queues, job progress, viewer state) lives in one small store per concern under `lib/` (`folderQueueStore.ts`, `verifyStore.ts`, `lbdirStore.ts`, `spectrogramStore.ts`, `attachmentsStore.ts`, `scraperLogStore.ts`, `lookupStore.ts`) — never one giant store. `ScreenLibrary.tsx`'s `useLibraryFilterStore` (both lenses' filters + view state) is persisted to localStorage (`lbb-library-filters`, Set-aware replacer/reviver) so the exact Library view survives app restarts (BUG-271). Data fetched from the backend is NOT put in zustand; it goes through `@tanstack/react-query` instead — whose cache is itself persisted for the four bulk keys (`collection-prefetch`, `library-catalog`, `library-performances`, `library-badges`) to IndexedDB via `PersistQueryClientProvider` + `idb-keyval` in `App.tsx` (7-day maxAge, buster `lbb-cache-v1`), so relaunches render instantly from the last snapshot while a staggered background refetch reconciles. `App.tsx` also persists the last route (`lbb-last-route`) and reopens on that screen at launch.
...
- **Command palette (Ctrl+K, TODO-263):** `components/CommandPalette.tsx`, mounted once in `AppShell`. One fuzzy box: type an LB number (`^(lb[-\s]?)?0*(\d{1,5})$` → "Go to LB-N" top hit → `/library?lb=N`), a screen name (fuzzy over the shared nav registry), or a date/venue fragment (debounced `GET /api/search`, ≥2 chars, stale-response guarded, degrades silently on backend error). `lib/navigation.ts` is now the **single source of truth for screens** — `NAV_GROUPS` + types moved out of `AppShell.tsx`; both the sidebar and the palette consume it, so curator-gated screens vanish from both when `curatorMode` is off. `lib/commandRegistry.ts` is the extension point: `registerCommands()`/`getCommands()` let future feature modules add palette actions (each `PaletteCommand.run` returns `void` to close or `Promise<string>` to show a footer outcome) with no palette-file edit — v1 ships nav commands + `action.checkUpdate` (`/api/flat_file/discover`). `lib/fuzzyMatch.ts` is the standalone scorer (subsequence match weighted by consecutive runs, word-start hits, position). SSE-backed actions (recompute, scans) are deferred to the activity center per the spec's D4. i18n keys under the top-level `palette` namespace.
...
| 2026-06-14 | v1.5.0 release: tapematch reliability fixes (TODO-139 tasks 2-7), qBittorrent save-path sync, hash-verified pipeline filing, LBDIR site recovery, mount drive stats, pipeline GUI fixes (BUG-166/172-182, TODO-282/143/145), Windows font fix (BUG-175); `gui_next/package.json` version bumped 1.4.0 -> 1.5.0. |
...
| 2026-06-12 | TODO-282: `get_mounts_with_stats()` adds `total` (capacity) and `used_pct` to each mount via `shutil.disk_usage()`; pipeline Collect step's `CollectDetail.tsx` MountPicker cards show "free of total" plus a colour-coded usage bar (warn ≥75%, bad ≥90%), updating reactively as the pipeline re-resolves the Collect step. |
| 2026-06-12 | TODO-282 follow-up: disk-usage calc extracted into `backend/filer.py` `get_disk_usage_stats(root_path, online)`, reused by `get_mounts_with_stats()` and `/api/collection/mounts` (now returns `free`/`total`/`used_pct` per mount); `ScreenMounts.tsx` MountCard shows the same "free of total" usage bar as the Collect step. |
| 2026-06-12 | TODO-285: backend uptime clock — new `GET /api/system/uptime` (`{uptime_seconds}`) sharing a single `_process_start_time` with `/api/admin/status`; About dialog's About tab shows a live HH:MM:SS uptime field next to version/build, to help confirm whether a backend restart actually happened. |
...
| 2026-05-26 | TODO-278/090: `dylan_performances` promoted from USER→MASTER (schema v3); new `lb_problems` MASTER table (id, lb_number, notes, added); 4 DB functions; `GET /api/performances` (date/lb/category filter); `GET/POST /api/lb_problems` + `PUT/DELETE /api/lb_problems/<id>` (curator-only write). |

...
| GET | `/api/tapematch/pairs?date=` | One date's `tapematch_pairs` rows for the similarity matrix: `{date, run_id, pairs: [{lb_a, lb_b, corr, emb_score, fp_score, same_family, similarity_pct, human_judgment, human_notes, ab_eligible, lb_says_same, lb_relation_text}]}`. `human_*`, `ab_eligible` (TODO-231/233) and `lb_says_same`/`lb_relation_text` read LIVE (best-effort) from observations.db; null on lock/missing. The LB-page claim is not in `tapematch_pairs` — it rides the same enrichment read so ScreenTapeMatchCuration's matrix can mark conflicts (`lb_says_same && !same_family`). `ab_eligible` is resolved per pair via `ab_clips.get_pair_source_info` (each pair's own latest common tapematch run) + the speed-kind and post-2026-07-06 run gates, matching what `POST /api/ab_clip` will accept — not the (possibly stale) run_id synced into `tapematch_pairs`. 200 with `pairs: []` for un-synced dates. |

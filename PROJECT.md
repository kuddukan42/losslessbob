...
| ScreenGaps | `screens/ScreenGaps.tsx` | Read-only "living Kokay list" at `/gaps` (Library nav group; 2026-07-17, TODO-256, `instructions/FABLE_GAPS_VIEW.md`). Decade-chip strip + year rows of small coverage cells (covered/partial/gap tones, future dimmed-outline) from `/api/gaps/summary` + lazy per-visible-year `/api/gaps/year/<year>`. Clicking a cell opens a right-hand detail pane (single-tab tab-group — §6 will add a Family-tree tab) via `/api/gaps/date/<iso>`: olof event(s), circulating entries (LB deep-links into Library), month-partial candidates, TapeMatch families. Empty state when `olof_events` is absent. |

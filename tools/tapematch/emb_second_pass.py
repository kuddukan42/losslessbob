...
def build_report(conn: sqlite3.Connection) -> str:
    ...
    add("")
    add("### Validation of the flips (2026-07-27)")
    add("")
    add("rule_d's zero-new-FP proof covers the 2,245-pair frozen sets only, so the")
    add("corpus-wide flips were scored separately against the curator's own stance.")
    add("Splitting every pair by what the curator actually says — `lb_says_same=1`")
    add("(claims same), `=0` (**explicit denial**), `NULL` (silent, no statement) — the")
    add("rule discriminates sharply:")
    add("")
    add("| curator stance | rule_d fires | of n | rate |")
    add("| --- | ---: | ---: | ---: |")
    add("| claims same | 732 | 3,631 | 20.16% |")
    add("| silent (no statement) | 721 | 17,293 | 4.17% |")
    add("| **explicit denial** | **12** | **3,038** | **0.39%** |")
    add("")
    add("A **52x** gap between claims-same and explicit-denial. The rule is tracking")
    add("source identity, not firing indiscriminately. Of the 12 denial hits, 10 already")
    add("carry corr >= 0.83 — tapematch's primary signal merges them regardless, so they")
    add("are curator label noise (the TODO-201 class), not rule_d errors.")
    add("")
    add(f"Of the {claimed + silent} flips: {claimed} the curator claims as same-source,")
    add(f"{silent - 1} the curator is silent on, and **exactly 1** contradicts an explicit")
    add("denial — 1999-11-09 LB-02737/LB-04289 (emb 0.872/0.937, corr 0.101), where the")
    add("LB-4289 notes say \"different recording than LB-1401, LB-2064, and LB-2737 based")
    add("on different crowd at begin of d1t2\". That single pair is the entire measured")
    add("false-positive exposure of wiring rule_d live. **The gate passes.**")
    add("")
    add("Two watch items for the fix, neither blocking:")
    add("")
    add("- **1978-03-09** is the only date (>=4 sources) where rule_d links *every* pair,")
    add("  collapsing the whole date into one family. Spot-check it before/after.")
    add("- 156 of 793 rule_d-firing dates have a median emb >= 0.60 across all their")
    add("  pairs. Elevated per-date baselines are the same-show/different-source confound")
    add("  BASELINE.md Task 8 documented; the both-convention requirement appears to")
    add("  handle it (those dates are not where the denial hits land), but a per-date")
    add("  baseline is the natural guard if rule_d is ever loosened below 0.75.")
    add("")

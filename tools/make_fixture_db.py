...
def cut_golden(src_db: Path, dates: set[str], dest: Path = GOLDEN_FIXTURE) -> dict[str, int]:
    ...
    # Two-show days are indexed "YYYY-MM-DD Early"/"Late", so match on the date prefix.
    tables["bobserve_event_index"] = rows(
        "bobserve_event_index", f"substr(date_str, 1, 10) IN ({','.join('?' * len(dates))})",
        list(dates))

...
Fix: the writer thread (_run) now closes its own connection once it drains the shutdown sentinel and sets self._conn = None; shutdown() only signals + joins and never touches self._conn. A writer that outlives the join timeout briefly leaks its connection, then closes it itself when the in-flight write finishes — safe; a segfault is not. Added TestWriteQueueShutdown (2 regression tests: normal shutdown is writer-owned; a write held past the join timeout is not freed by the caller and shutdown does not raise). Verified: full suite 905 passed locally; real GitHub Actions run went green (backend-tests, backend-smoke, gui-check all passed on CI run 29872979915) — the same commit's prior run without this fix segfaulted backend-tests twice (run 29871749712, exit 139).
